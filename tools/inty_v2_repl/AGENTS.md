...
- **User online signal (`user_signed_on`)**: Product intent is anthropomorphic: the companion should act as if it *noticed* the user entering the channel and may greet first (see repo `/docs/FR_USER_SIGN_ON_GREETINGS.md`). On the **first** successful WebSocket connect, if the URL includes `agent_id`, the bridge sends one **`{"type":"user_signed_on","agent_id":...}`** control frame (not repeated per chat turn or on reconnect within the same REPL process). Interactive `repl` prints stderr lines for send and ack (before the `>` prompt).
