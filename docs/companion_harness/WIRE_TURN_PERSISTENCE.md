...
本文用三层模型归纳 **伴侣 WebSocket 聊天** 上各类输入/输出的归属：传输帧、回合信封、持久化落点。适用于讨论「某事件会不会进 transcript」「和 `chat_history` 是否一致」「`CHAT_LOGS.md` 是什么」等架构问题。
...
| **E** | Agent 非对白轨迹 | `ai_private*`、`CHAT_LOGS.md`、`.companion_runtime_events.jsonl` |
| **F** | Agent 状态与记忆 | `IDENTITY`/`SOUL`/…、`context.json`、压实/schedule 快照等（非逐轮 log） |
| **G** | 客户端可见历史 | 产品 API 的 `chat_history`（应与 D 语义对齐，结构不同） |

**注意**：`transcript.jsonl` 只是 **D** 的主轨，不等于全部「agentic logs」；完整 agent 时间线还需 **D 副轨 + E + F**，客户端时间线看 **G**。
...
| **Structured logs（结构化日志）** | 追加一行/一条记录，审计或排障 | E：`CHAT_LOGS.md` 行；`runtime_events` 的 `kind`+`ts` |

---

## `CHAT_LOGS.md`（属 E，不是 B）

**写入方**：仅 `user_signed_out`、`ws_conn_dropped` 处理路径（`append_companion_chat_logs_line_for_ws_control`）。

**格式**：每事件一行英文 Markdown 列表项，带反引号字段（`utc_ts`、`user_id`、`chat_id`、`agent_id`、`received_message_uuid`；drop 另有 `client_dropped_at_utc`、`ws_close_code` 等）。

**不写入**：`user_signed_on`、`ping`、`client_context`、各 `ack`。

**与 B 的区别**：B 是回合内字段、常随连接内存（`tc_box`）消费；`CHAT_LOGS.md` 是 **跨连接追加的审计文档**，默认不进 LLM 对话窗口。
...
| `user_signed_on` | RAM `heartbeat_context`；问候轮 `ImplicitSignalBundle.user_signed_on`（B） | D：synthetic user + assistant；G：仅 assistant；0×`CHAT_LOGS` |
| `user_signed_out` | 无 B | E：`CHAT_LOGS` 一行；0×D |
| `ws_conn_dropped` | 无 B | E：`CHAT_LOGS` 一行；0×D |
| 用户 `chat` 帧（C） | C + B（时间等） | G + D |
| 服务端 `inner_tick` | 无 Wire | D 或 `transcript_inner_tick.jsonl` |
| LLM/工具失败 | 无 Wire | E：`runtime_events.jsonl` |
...
```
  ...
  LAYER 2: TURN ENVELOPE (B / C)       In-memory + per-turn — not CHAT_LOGS
  ──────────────────────────────
       ┌─────────────────────────────────────────────────────────────────┐
  ...
  ────────────────────────────────────
       ...
       │
       E CHAT_LOGS.md          audit lines: user_signed_out, ws_conn_dropped
       E .companion_runtime_events.jsonl   llm_inference_failure, tool_background_failure, …
  ...
  user_signed_out         Wire ──► E: 1 line in CHAT_LOGS.md
                          └──► (0 × B, 0 × D transcript row)

  ws_conn_dropped         Wire ──► E: 1 line in CHAT_LOGS.md
                          └──► (0 × B, 0 × D)
  ...
  LLM / tool failure    (runtime) ──► E: runtime_events.jsonl
