...
- **与 tool_background 的时序**：dreaming batch 在 LivingSphere compact 前会等待本会话 `tool_bg_idle`（与下一轮 `run_turn` 开头等待同一 Event），以便 `living_sphere_record_update` 在异步工具线程里先 append jsonl；等待上限由 `app.features.companion_tool_bg_idle_wait_timeout_sec`（默认 120s）配置。
