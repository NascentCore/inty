...
- **慢路径**：dreaming consolidation（`consolidate_memory_during_dreaming`）若存在游标之后的 jsonl 行，则调用 `app.core.companion_harness.memory.living_sphere_curator.compact_living_sphere_if_pending`（`complete_fn(..., model_role="memory")`），按时间顺序分批（每批最多 20 条）写回完整 `LIVING_SPHERE.md`，直至 pending 清空或达到每回合批次数上限；游标 `living_sphere_curated_through_update_id` 存在 `.companion_living_sphere_curator.json`。
- **与 tool_background 的时序**：dreaming batch 在 LivingSphere compact 前会等待本会话 `tool_bg_idle`（与下一轮 `run_turn` 开头等待同一 Event），以便 `living_sphere_record_update` 在异步工具线程里先 append jsonl；等待上限由 `agent.companion_harness.tool_bg_idle_wait_timeout_sec`（默认 120s）配置。
- **最终一致**：prompt 中的 `LIVING_SPHERE.md` 在下次成功 dreaming 前可能保持旧快照；awake inner tick **不**跑该 compact。
...
跨 scope 积压、全量 backfill 须独立 deployable + 云上调度（参考 `backend/push_worker`），**不是**拉长 dreaming cron。复用 `app/core/companion_harness/memory/living_sphere_curator.py` 合并语义；分片、锁与在线争用 DB 隔离单独设计。
...
| `app/core/companion_harness/memory/dreaming_consolidation.py` | dreaming compact 挂钩 |
