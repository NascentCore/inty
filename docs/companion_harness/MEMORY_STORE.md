...
- 不在范围：跨 transport / turn / tool 的整体职责切分 —— 见 [`ARCH.md`](/docs/companion_harness/ARCH.md)；Wire / Turn / Persistence 与 A–G 落点对照见 [`WIRE_TURN_PERSISTENCE.md`](/docs/companion_harness/WIRE_TURN_PERSISTENCE.md)。
...
- **`.companion_runtime_events.jsonl`**：WS lifecycle（`user_signed_on/out`、`ws_conn_dropped`）与 LLM/工具失败等 runtime 行；默认不进 prompt 对话窗。字段与落点见 [`WIRE_TURN_PERSISTENCE.md`](/docs/companion_harness/WIRE_TURN_PERSISTENCE.md) 的 E 落点速查。
...
- [Wire → Turn → Persistence](/docs/companion_harness/WIRE_TURN_PERSISTENCE.md)
