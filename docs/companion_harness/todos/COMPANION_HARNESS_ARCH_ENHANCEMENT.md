...
| 版本化正文集合 | `SessionCorpus` | 某 binding 下按 `corpus_rel_key` 存储、读 head 的正文与快照（IDENTITY / SOUL / USER / MEMORY、transcript、context_json、memory 树等）。对应实现面主要是 `MemoryStore` + `companion_memory_document_versions`。 |
