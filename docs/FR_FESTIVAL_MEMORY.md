...
- **memory 表**：沿用现有表，新增 JSON 列 **`metadata`**（节日记忆存储 `{"festival_name": string, "festival_data": "YYYY-MM-DD", "llm_config": {model, temperature, max_tokens} 可选；"llm" 为历史字段}`，兼容 `festival_date` key），`memory_type` 取值包含 `festival`。节日记忆语义：`(user_id, agent_id, festival_name, festival_date)` 唯一确定一条，`content` 存该用户与该角色在该节日下的回忆摘要。`festival_name`/`festival_date` 旧列保留用于回退兼容；读取时优先 `metadata`。**`delivery_at`**（DateTime with timezone，可空）：节日记忆提示**首次**投递到会话的时间；`NULL` 表示尚未投递，仅在用户发起聊天或拉取消息列表时按需写入 chat_history 并更新该字段。`extracted_at` 已废弃，不再作为读路径依赖字段。
