...
| 「生成 API key」 | 当前实现使用 **JWT（Bearer）** 校验用户身份（与 App 其它接口一致），**没有**面向第三方的专用静态 API key 机制。常见做法：创建邮箱密码用户后，用 **登录换短期 JWT**（**2.1**）或 **`scripts/generate_long_term_token.py` 签发长期 JWT**（**2.2**，便于交付客户联调）。对外仍建议称「Bearer token / JWT」。 |
...
**结论**：可行。该脚本按用户标识从数据库查出用户后，调用 `create_access_token(..., expires_delta=timedelta(days=...))` 打印 **标准 JWT**，与登录接口签出的 token 在鉴权层面一致，仅 **过期时间更长**，适合作为「交给客户联调的长期 Bearer」口头上的 API key（本质仍是 JWT）。
...
客户侧拿到 token 后，与 **2.1** 换得的短期 JWT 用法相同：`GET /api/v1/live-chat/status` 与 WebSocket 均带同一 `Authorization: Bearer`。

## 2.3 测试 Voice Call SDK（Android）与最小端到端验证

以下假设已在 **dev** 准备好：`agent_id`、长期或短期 Bearer token、`wss` 基地址与 `https` 基地址一致（仅 scheme 不同）。
...
3. 使用 `IntyVoiceCallUrls.liveChatWebSocketUrl(wssBase, agentId, token, ...)` 得到 URL 时，**优先**在 WebSocket 握手请求上增加 `Authorization: Bearer <token>`，避免仅依赖 URL 中的 `token=` 查询参数（减少泄漏面）。
...
| 6 | Inty 运营 | 通过安全渠道一次性交付：`INTY_DEV_BASE`、`agent_id`、Bearer token（**2.2**）；邮箱密码仅在客户需要自己换票时提供（**2.1**）。 |
| 7 | 客户 | 按 **2.3** 执行 HTTP smoke，再集成 SDK 做语音 E2E；问题反馈带上 `agent_id`、大致时间与是否收到 `4001`/`4003`/业务 `error` JSON（勿回传完整 token）。 |
