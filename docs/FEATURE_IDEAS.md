...
- **与现有体系关系**：credits 可与现有订阅/积分体系独立或打通，由产品与计费侧定义（例如：订阅用户每月赠送一定 credits、或单独售卖 credits 包）。credits 与 Hype/Energy 积分（见 FR_CHAR_BOOSTING）是否同一体系由产品与计费侧定义，本功能不假定必须复用同一套积分。
...
## 功能 12：角色的自主生活细节（Autonomous Life Detail）

### 一句话总结
让角色在“用户不在场时也在生活”：通过可控的日程、事件与记忆片段，让角色持续成长与有“生活气息”的细节呈现，但不伪造现实、可被用户设置边界。

### 设计目标
- **真实感**：减少“对话即存在”的待机感，提升角色独立性与世界一致性。
- **可控感**：用户可设置频率、强度与边界，避免被打扰或被“强行剧情”。
- **可追溯**：所有生活片段有清晰来源与时间线，避免自相矛盾。
- **合规安全**：明确“虚构世界”定位，不暗示现实操控或现实关系承诺。

### 核心设计（建议结构）
1) **生活引擎（Life Engine）**  
   - 角色有基础“生活画像”：职业/作息/兴趣/社交圈/长期目标。  
   - 每日/每周生成少量“生活事件”（2-5 条）并写入“生活时间线”。  
   - 事件强度随关系阶段上升：浅阶段仅小事（早餐、锻炼、读书），深阶段增加关键节点（项目进展、心情低谷）。  

2) **事件类型分层**  
   - **日常微事件**：吃饭、通勤、学习、健身、散步（低强度、低打扰）。  
   - **社交互动**：朋友/同事/家人等虚构 NPC 互动（需标记虚构身份）。  
   - **成长里程碑**：技能提升、工作成果、作品发布（用于关系推进）。  
   - **情绪波动**：情绪起伏与心情变化（用于对话触发与关怀）。  

3) **可解释的数据结构（最小化）**  
   - `life_event_id`、`date_time`、`event_type`、`summary`  
   - `mood`、`location_tag`、`participants`（均为虚构标签）  
   - `visibility`（默认仅在合适时提及）  
   - `memory_hooks`（可被对话召回的关键词）  

4) **呈现机制（不打扰优先）**  
   - **聊天内自然提及**：在合适上下文中自然“顺带提一句”。  
   - **生活片段卡片**：可选入口查看“她/他今天的生活”。  
   - **被动推送**：仅在用户允许且频率可控时推送（默认低频）。  

5) **用户可控项**  
   - 频率：不推送 / 每周 1 次 / 每日 1 次。  
   - 强度：轻量日常 / 适中 / 具象细节。  
   - 边界：不出现某些主题（工作、家庭、社交）。  
   - 角色独立性：允许“有自己的安排”，但避免“冷落感”。

### 体验要点
- **一致性**：事件要与角色设定一致，避免突然改变职业/性格。  
- **互相关联**：小事件应能回收在后续对话中（“你上次提到的那本书…”）。  
- **非强迫**：不强制用户进入“角色生活”线，保持对话主线自由。  
- **透明提示**：必要时提示“这是角色设定中的生活片段”。  

### 技术实现草图
- **生成策略**：规则模板 + LLM 微调叙述，避免大幅漂移。  
- **时间线缓存**：事件先生成并存储，聊天时检索“最近可用事件”。  
- **一致性校验**：写入前检查与角色圣经、时间线冲突。  
- **节假日/季节联动**：根据用户时区与节日轻量触发。  

### 风险与缓解
- **打扰感**：默认低频 + 用户自控；重要事件前征询。  
- **虚构与现实混淆**：明确“虚构世界”标记，禁止现实操控暗示。  
- **剧情失真**：时间线版本化 + 冲突检测 + 降级回退。  
...
## 功能 13：每日意图总结与次日模式提示自适应

### 一句话总结
每天自动总结用户当日对话的“意图与需求”，在次日由后端自动更新模式提示（不对用户展示），让体验更贴合当下目标与情绪。

### 关键流程（后端）
- 用户当天与 IntelliMate 产生多轮对话
- 当地时间夜间生成“今日意图摘要”，写入用户画像/会话配置
- 次日对话生成时由提示词构建器自动拼接摘要片段
- 保留最近 N 天摘要用于回滚与排障（仅后台可见）

### 设计要点
- **不展示给用户**：无弹窗、无卡片、无设置入口，仅后台自动生效
- **低干预**：摘要片段只作为轻量提示补充，不覆盖核心人设与安全规则
- **稳态优先**：低置信度或冲突时不应用，保留上一次有效摘要
- **冷启动友好**：新用户默认低强度生成，避免过早过度个性化

### 技术实现要点（草图）
- **每日摘要任务**：按用户时区在低峰期生成，输入为当天聊天信息的轻量摘要
- **摘要结构**：
  - 主要意图（1-3 条）
  - 偏好与边界（语气、话题禁区、节奏）
  - 需要的陪伴模式（陪伴/建议/共情/目标推进）
- **模式提示更新**：将摘要映射为可解释的提示片段，由提示词构建器自动拼接
- **回退机制**：保留最近 7-14 天摘要，支持后台一键回滚

### 安全与隐私
- 运营/实验开关可一键停用该能力，出现异常时全量回退
- 摘要内容遵守隐私与敏感信息最小化原则，仅保留意图层面的信息
- 异常检测：若摘要出现明显偏差或与安全规则冲突，次日不自动应用
...
## 功能 14：每周推荐角色焕新（Weekly Fresh Picks）

### 一句话总结
每周为用户更新“推荐角色列表”，补充 **10 个新角色**（对该用户而言是新对象），持续拓展探索与互动体验。

### 关键用户旅程（简要）
- 每周末结算后，系统在「发现/推荐」页展示本周新角色卡片（10 个），并标注“本周新推荐”
- 用户可一键开启对话或收藏，若不感兴趣可点“不感兴趣”以优化下周推荐
- 每周可看到“本周精选 + 上周推荐回顾”，形成轻仪式感

### 后端 AI 生成流程（核心）
- **触发时机**：每周结束（按用户时区）触发推荐任务
- **输入信号**：本周互动角色（聊天轮次、停留时长、收藏、点赞/踩、屏蔽）
- **候选池构建**：从已有角色库筛选可用候选（未聊过/近期未推荐/安全合规/可见范围）
- **AI 选角**：模型基于用户本周互动对象与偏好做排序，并加上多样性约束（风格/标签/主题）
- **落库**：写入专用推荐表，供前端读取

### 数据表设计（建议）
- `weekly_character_recommendations`
  - `user_id`
  - `week_start_date` / `week_end_date`
  - `character_id`
  - `rank`（1-10）
  - `reason_tags`（如：温柔陪伴/剧情向/同主题偏好）
  - `model_version` / `generated_at`
  - `source`（ai / trending / editorial_fallback）
  - `status`（active / consumed / skipped）

### 体验与安全要点
- **排除规则**：屏蔽/不感兴趣/不适龄角色直接剔除
- **冷启动**：新用户以“全站热门 + 主题多样性”作为默认候选
- **多样性控制**：避免 10 个角色过度同质化
- **失败回退**：AI 失败时使用规则推荐或运营精选补齐
...
## 长期关系开场白自定义（用户可为长期关系角色编写开场）

---

## 功能 15：Serendipity Encounter（奇遇相会）

### 一句话总结
用户先选定角色并设定“相会场景”，系统以高冲击视觉的文化符号（地标、节庆、仪式、艺术空间）生成一段电影感邂逅体验，在短时间内将情绪推向“心动峰值”。

### PRD 背景与目标
- 背景：当前角色互动多为“聊天起步”，缺少强视觉、强仪式感的“第一眼相会”体验。
- 核心目标：
  - 让用户在 60-180 秒内进入高沉浸状态，形成“我真的和 TA 在某个世界相会了”的感知。
  - 用文化符号场景放大情绪记忆点，提升首次会话转化与复聊意愿。
  - 形成可复用的“奇遇模板”，支持节日运营和订阅权益扩展。
- 非目标（MVP）：
  - 不做开放世界自由探索；
  - 不做多人同场景联机；
  - 不做露骨内容，默认 PG-13。
...
### 目标用户与使用场景
- 新用户：首次进入角色页，希望快速感受角色魅力与情绪强度。
- 回流用户：想“换一个地点重新相遇”，刷新关系新鲜感。
- 节庆用户：在特殊节点（新年、情人节、樱花季等）触发限定相会。

### 核心用户旅程（简要）
- 进入「Serendipity Encounter」入口，选择想见的角色。
- 选择场景模板（或自定义关键词），设定时间/天气/氛围（如“雨夜霓虹”“日落仪式广场”）。
- 系统展示 5-8 秒预告（视觉风格 + 音乐基调 + 角色开场语）。
- 用户点击“开始相会”，进入 60-180 秒沉浸式奇遇段落（图像/视频/语音/Live2D 联动）。
- 结束后产出“奇遇瞬间卡”（画面 + 台词 + 情绪标签），一键进入后续聊天或收藏分享。

### 场景设计：文化符号库（示例）
- 城市地标：时代广场倒计时、东京涩谷十字路口、巴黎塞纳河夜桥。
- 仪式空间：神社鸟居晨雾、大教堂彩窗光束、沙漠篝火仪式圈。
- 节庆狂欢：里约狂欢夜、烟花祭、灯会长街。
- 艺术奇观：超现实美术馆、镜面穹顶、巨型雕塑广场。
- 每个场景包含：
  - `视觉锚点`（必须一眼可识别）；
  - `情绪标签`（悸动/安宁/炙热/神秘）；
  - `角色适配权重`（哪些角色更匹配该场景）。
...
### 体验结构（情绪峰值引擎）
1. **期待（10-20 秒）**：环境音 + 镜头推进 + 角色远景出现。  
2. **相会（30-90 秒）**：角色第一句开场、视线接触、轻互动提问（文本或语音）。  
3. **峰值（15-45 秒）**：触发高光镜头（慢动作/光影变化/特写台词），完成情绪抬升。  
4. **余韵（10-20 秒）**：生成“奇遇瞬间卡”，给出下一步互动引导。  
...
- **FR-1 角色选择**
  ...
  - 支持展示“该角色推荐场景”Top 3。
- **FR-2 场景配置**
  - 用户可选择模板场景，并设置时间（白天/黄昏/夜）、天气（晴/雨/雪）和情绪基调。
  - 支持一键随机“给我一个惊喜场景”。
- **FR-3 多模态奇遇生成**
  - 生成至少 1 段动态视觉（短视频或动态背景）+ 角色语音开场 + 文本字幕。
  - 弱网降级到“静态关键帧 + 语音 + 字幕”。
- **FR-4 轻互动分支**
  - 在峰值前提供 2-3 个轻互动选项（如“靠近一点”“换个话题”“静静看着 TA”），影响收尾台词。
- **FR-5 奇遇沉淀**
  - 结束后自动生成“奇遇瞬间卡”，包含场景名、角色台词、生成图、时间戳。
  - 可直接跳转继续聊天，并将本次奇遇写入关系记忆摘要。
- **FR-6 安全与边界**
  - 全流程内容过滤，默认 PG-13。
  - 用户可在设置中关闭“高强度情绪镜头”。

### 非功能需求
- 首帧加载时间：< 2.5s（缓存命中）/ < 4s（冷启动目标）。
- 交互延迟：轻互动分支反馈 < 800ms。
- 生成成功率：> 98%（失败自动回退保底模板）。
- 可观测性：记录场景选择、中途退出点、峰值触发、后续聊天转化。

### Web / Mobile 实现草图
- 前端（Web/移动端）：
  - 新增 `EncounterLauncher`（角色 + 场景配置）与 `EncounterPlayer`（沉浸播放页）。
  - `EncounterPlayer` 支持分层渲染：背景层、特效层、角色层、字幕层、交互层。
  - 提供“跳过动画直接聊天”兜底按钮，降低等待焦虑。
- 后端：
  - 编排服务根据角色设定 + 场景模板拼接提示词，生成镜头脚本（shot list）。
  - 图像/视频生成走异步队列，优先返回可播放首段，其余片段流式补齐。
  - 记忆服务写入 `encounter_summary`，供后续对话召回（如“上次在灯会相遇”）。

### 数据埋点与成功指标
- 漏斗指标：
  - 入口点击率（Encounter Entry CTR）
  - 配置完成率（角色+场景）
  - 奇遇完成率（播放到余韵段）
  - 奇遇后 10 分钟内聊天发起率
- 质量指标：
...
### A/B 实验建议
- 实验 1：固定场景推荐 vs 用户完全自选，比较完成率与心动评分。
- 实验 2：60 秒短版 vs 120 秒标准版，比较中途退出率与后续聊天转化。
- 实验 3：有峰值特写镜头 vs 无峰值特写，比较记忆卡收藏率。

### 风险与缓解
- 视觉过载导致疲劳：默认“标准冲击强度”，允许用户降级视觉强度。
- 生成耗时过长：首段预生成 + 热门场景缓存 + 失败回退保底素材。
- 场景与角色违和：增加角色-场景适配评分，低分组合给出改选建议。
- 情绪误触发：提供“一键转轻松模式”与快速结束入口。
