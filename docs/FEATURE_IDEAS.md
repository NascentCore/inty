...
### 一句话总结
每天自动总结用户当日对话的“意图与需求”，在次日以可控、可见、可编辑的方式更新模式提示，让体验更贴合当下目标与情绪。

### 关键用户旅程（简要）
- 用户当天与 IntelliMate 产生多轮对话
- 当地时间夜间生成“今日意图摘要”（不打扰式，可在设置查看）
- 次日首次进入对话时展示“模式更新卡片”：一键接受 / 编辑 / 忽略 / 关闭
- 用户可随时在设置中查看与回退最近 N 天的意图摘要

### 体验要点
- **透明可控**：用户能看到“摘要内容 + 影响点”，避免暗改提示词
- **可编辑**：支持手动修正摘要中的偏差或增加边界
- **轻量提示**：仅在次日首次进入时展示一次，不影响当日对话流
- **冷启动友好**：对新用户默认低强度生成，避免过早过度个性化
...
- **模式提示更新**：将摘要映射为可解释的提示片段（可对比查看变更）
- **回退机制**：保留最近 7-14 天摘要，可一键恢复到任一日

### 安全与隐私
- 提供**关闭开关**与“仅本地摘要/不入库”选项（若平台支持）
- 摘要内容遵守隐私与敏感信息最小化原则，仅保留意图层面的信息
- 异常检测：若摘要出现明显偏差，次日默认不自动应用而需确认
