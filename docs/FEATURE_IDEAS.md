...
### 一句话总结
每天自动总结用户当日对话的“情感意图与陪伴需求”，在次日由后端自动更新模式提示（不对用户展示），让亲密陪伴更贴合当下情绪与关系阶段。

### 关键流程（后端）
- 用户当天与 IntelliMate 产生多轮对话
- 当地时间夜间生成“今日陪伴摘要”，写入用户画像/会话配置
- 次日对话生成时由提示词构建器自动拼接摘要片段
- 保留最近 N 天摘要用于回滚与排障（仅后台可见）

### 设计要点
- **不展示给用户**：无弹窗、无卡片、无设置入口，仅后台自动生效
- **低干预**：摘要片段只作为轻量陪伴提示补充，不覆盖核心人设与安全规则
- **稳态优先**：低置信度或冲突时不应用，保留上一次有效摘要
- **关系一致性**：优先维护亲密关系节奏与边界，避免忽冷忽热的语气漂移
...
- 异常检测：若摘要出现明显偏差、与安全规则冲突或触发亲密边界预警，次日不自动应用
