...
**Voice call** (from Chat): Voice calls require microphone permission. The centre circle shows connection status (e.g. Connecting / Connected), **listening** when the AI is waiting for you, or **speaking** when the AI is talking—then you see “tap to interrupt AI” and a wave animation; tap the circle to interrupt and speak. Use mute and end-call as needed. After a call ends, IntelliMate saves local call audio and shows replay controls in voice-call history bubbles. Matching now uses both session id and turn id: multiple audio chunks from one AI turn are associated to that turn’s transcript text, reducing cross-turn mismatch. Transcript fragment play buttons prioritize local saved audio clips for playback. Tap the bubble to expand/collapse transcript text messages. If the voice call screen stays blank, grant microphone permission when prompted or in system Settings → App → IntelliMate → Permissions.
