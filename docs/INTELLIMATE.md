...
You can also tap **Test my MBTI type** above the input box to prefill a structured MBTI interview prompt that helps the assistant run a step-by-step type discovery conversation.
...
| Start an MBTI test chat | Official Assistant Chat → **Test my MBTI type** | Prefills a structured MBTI interview prompt (not auto-sent), then continue by answering each follow-up question. |
