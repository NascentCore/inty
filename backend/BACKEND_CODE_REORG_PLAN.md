...
## 里程碑
1. **目录搭建**
   - 创建 `backend/`，逐步迁入后端服务目录（`app/`、`tools/scripts/`、`devops/` 等）。
   - 迁移同时保留原路径的引用映射记录，便于后续批量替换。
   - 进展：2025-11-16 已将根目录的 `README.md`、`AGENTS.md`、`TODOS.md` 与 `docs/` 迁入 `backend/`；2026-05-13 已将 `alembic/` 迁入 `backend/alembic/`。
2. **脚本与配置同步**
