...
async def cancel_evaluation_session(
    *,
    ...
    try:
        ...
        logger.info("用户 {} 取消评测会话: {}", current_user.id, session_id)
        return {"success": success, "message": "评测会话已取消"}

    except Exception:
        logger.exception("取消评测会话失败")
        raise HTTPException(
...
async def parse_questions_file(
    *,
    ...
    try:
        ...
        logger.exception("解析问题文件失败")
        raise HTTPException(
            status_code=400, detail="Failed to parse file"
        ) from None
...
async def create_evaluation_agent(
    *,
    ...
    try:
        ...
        logger.info("用户 {} 创建评测智能体: {}", current_user.id, agent.id)
        return agent
...
async def update_evaluation_agent(
    *,
    ...
    try:
        ...
        logger.info("用户 {} 更新智能体: {}", current_user.id, agent_id)
...
async def delete_evaluation_agent(
    *,
    ...
    try:
        ...
        logger.info("用户 {} 删除智能体: {}", current_user.id, agent_id)
...
async def create_evaluation_template(
    *,
    ...
    try:
        ...
        logger.info("用户 {} 创建评测模板: {}", current_user.id, template.id)
        return template

    except Exception:
        logger.exception("创建评测模板失败")
        raise HTTPException(
    ...
    *,
    ...
    try:
        ...
        logger.info(
            "用户 {} 导出评测结果: {} 个会话",
            current_user.id,
            len(export_request.session_ids),
        )
...
async def compare_evaluation_sessions(
    *,
    ...
    try:
        # 这里应该实现实际的对比逻辑
        # 暂时返回模拟数据
        logger.info("用户 {} 对比评测会话: {}", current_user.id, session_ids)
...
async def get_llm_latency_trend(
    *,
    ...
    try:
        ...
        logger.exception("获取 LLM 延迟趋势失败")
        raise HTTPException(
...
async def get_live_chat_latency_trend(
    *,
    ...
    try:
        ...
        logger.exception("获取 Live Chat 延迟趋势失败")
        raise HTTPException(
...
async def get_live_chat_basic_stats(
    *,
    ...
    try:
        ...
        logger.exception("获取 Live Chat 基础统计失败")
        raise HTTPException(
...
async def get_all_agents_image_counts(
    *,
    ...
    try:
        ...
        logger.debug("获取所有角色图片数量，共 {} 个角色有图片", len(counts))
        return {"counts": counts}

    except Exception:
        logger.exception("获取角色图片数量失败")
        raise HTTPException(
...
async def get_agent_generated_images(
    *,
    ...
    try:
        ...
        logger.debug("获取角色 {} 的生成图片，共 {} 张", agent_id, len(images))
        return {"images": images, "total": len(images)}

    except Exception:
        logger.exception("获取角色生成图片失败")
        raise HTTPException(
