...
def upgrade() -> None:
    # 1. 添加 title 字段
    op.add_column(
        "user_notifications", sa.Column("title", sa.Text(), nullable=True)
    )

    # 2. 重命名 rendered_content 为 content
    op.alter_column(
        "user_notifications", "rendered_content", new_column_name="content"
    )
...
def downgrade() -> None:
    ...
    op.alter_column(
        "user_notifications", "content", new_column_name="rendered_content"
    )
