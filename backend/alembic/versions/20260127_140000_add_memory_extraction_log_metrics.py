...
def upgrade() -> None:
    op.add_column(
        ...
        sa.Column("duration_seconds", sa.Float(), nullable=True, comment="当次抽取总耗时秒"),
    )
    op.add_column(
        "memory_extraction_log",
        sa.Column("prompt_tokens", sa.Integer(), nullable=True, comment="LLM 输入 token 数"),
    )
    op.add_column(
        "memory_extraction_log",
        sa.Column("completion_tokens", sa.Integer(), nullable=True, comment="LLM 输出 token 数"),
    )
