...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "message_content", sa.Text(), nullable=True, comment="生成的Agent消息内容"
        ),
        sa.Column(
            "sent_at", sa.DateTime(timezone=True), nullable=False, comment="发送时间"
        ),
        sa.Column(
            "created_at",
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_push_notification_history_sent_at", table_name="push_notification_history"
    )
    op.drop_index(
        "ix_push_notification_history_agent_id", table_name="push_notification_history"
    )
    op.drop_index(
        "ix_push_notification_history_user_id", table_name="push_notification_history"
    )
    op.drop_index(
        "ix_push_notification_history_chat_id", table_name="push_notification_history"
    )
    op.drop_index(
        "ix_push_notification_history_id", table_name="push_notification_history"
    )
