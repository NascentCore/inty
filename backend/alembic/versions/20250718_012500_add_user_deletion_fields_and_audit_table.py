...
def upgrade() -> None:
    ...
    op.create_table('user_deletion_logs',
        ...
        sa.Column('original_user_data', postgresql.JSONB(astext_type=sa.Text()), nullable=True, comment='原始用户数据快照'),
        sa.Column('deletion_reason', sa.String(255), nullable=True, comment='删除原因'),
        sa.Column('deletion_type', sa.String(50), nullable=False, default='user_requested', comment='删除类型：user_requested, admin_deletion, compliance'),
        sa.Column('anonymized_fields', postgresql.ARRAY(sa.String()), nullable=True, comment='已匿名化的字段列表'),
        sa.Column('subscription_status_at_deletion', sa.String(50), nullable=True, comment='删除时订阅状态'),
        sa.Column('related_data_action', sa.String(100), nullable=True, comment='关联数据处理方式'),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False, comment='日志创建时间'),
    ...
    op.create_index('ix_user_deletion_logs_created_at', 'user_deletion_logs', ['created_at'])
