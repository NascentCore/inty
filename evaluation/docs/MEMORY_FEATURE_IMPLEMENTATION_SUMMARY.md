...
- 管理员在 evaluation 侧边栏「节日记忆提取」页面配置节日（名称、节日日期）与提示词，以及**执行日期**、**执行时刻（UTC 小时 0–23）**、可选**窗口内最少用户消息数**（默认 15）；执行日期不能早于节日日期。可立即执行或由定时任务对「在节日当天 0 点至次日 4 点（UTC）28 小时内用户消息达到配置的窗口内最少条数（默认 15）以上」的 (用户, 角色) 组合抽取节日回忆并写入 `memory` 表。
