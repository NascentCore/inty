...
Inty 评估（体育与运营工具），基于 React/TypeScript 和 Vite 构建。
目前正在建设中，已被吊运至起重机`FastAPI` 静态目录，并由后端统一在 `/evaluation`路由提供访问。

- **⚠️注意：操作所有权的均为同一份数据，使用同样的 API 密钥（仅用于开发环境）。请勿泄露或在公网展示。**
...
```方式二：使用 Docker 构建（多阶段构建自动圣诞老人并入静态资源）```bash
...
# 请提交改动，创建 PR 并合并
```译文参考：`https://github.com/NascentCore/inty/pull/655/files`

## 环境指标说明

- 集成构建（生产访问）：构建时不注入环境变量，引入使用相对路径访问仓库API。
...
- 其他目录：
  - `components/`、`hooks/`、`services/`、`utils/`## LangSmith 调试

在单角色聊天页面点击LangSmith图标查看车队；如未显示，请刷新页面。
...
## 故障排查（FAQ）- 端口被占用：`http://localhost:3000` 必须使用 3000 端口；`评估/开始。sh` 会尝试终止占用进程
...
- FastAPI 路由：`app/main.py`
  ...
  - 资源访问：`GET /evaluation/{path}`返回静态文件
- Docker多阶段构建：`Dockerfile`
  - 第一阶段构建前端并将产物置于 `/app/static/evaluation/`
  - 第二阶段复制上述产物到后端镜像的 `app/static/evaluation/`## 光标摘要

- 技术栈：React + TypeScript + Vite；浏览器直连仓库
- 集成访问：参与者在`/evaluation` 提供页面与静态资源
