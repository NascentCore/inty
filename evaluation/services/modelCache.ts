...
class ModelCacheService {
  ...
  async getOpenRouterModels(forceRefresh = false): Promise<OpenRouterModel[]> {
    // 检查缓存
    if (!forceRefresh) {
      const cached = this.getCachedModels(CACHE_KEYS.OPENROUTER_MODELS, CACHE_KEYS.OPENROUTER_MODELS_TIME);
      if (cached) {
    ...
    try {
      ...
      // 缓存API结果
      this.cacheModels(models, CACHE_KEYS.OPENROUTER_MODELS, CACHE_KEYS.OPENROUTER_MODELS_TIME);
  ...
  async getScoringModels(forceRefresh = false): Promise<ScoringModel[]> {
    try {
      // 检查缓存
      if (!forceRefresh) {
        const cached = this.getCachedModels(CACHE_KEYS.SCORING_MODELS, CACHE_KEYS.SCORING_MODELS_TIME);
        if (cached) {
      ...
      // 缓存结果
      this.cacheModels(models, CACHE_KEYS.SCORING_MODELS, CACHE_KEYS.SCORING_MODELS_TIME);
      ...
      // 尝试返回缓存的数据
      const cached = this.getCachedModels(CACHE_KEYS.SCORING_MODELS, CACHE_KEYS.SCORING_MODELS_TIME, true);
      if (cached) {
   ...
   */
  getCacheStatus() {
    const openRouterCacheTime = localStorage.getItem(CACHE_KEYS.OPENROUTER_MODELS_TIME);
    const scoringCacheTime = localStorage.getItem(CACHE_KEYS.SCORING_MODELS_TIME);
    
    return {
      openRouter: {
        hasCache: !!openRouterCacheTime,
        cacheTime: openRouterCacheTime ? new Date(parseInt(openRouterCacheTime)) : null,
        isExpired: openRouterCacheTime ? Date.now() - parseInt(openRouterCacheTime) > CACHE_EXPIRY : true,
      },
      scoring: {
        hasCache: !!scoringCacheTime,
        cacheTime: scoringCacheTime ? new Date(parseInt(scoringCacheTime)) : null,
        isExpired: scoringCacheTime ? Date.now() - parseInt(scoringCacheTime) > CACHE_EXPIRY : true,
      },
    };
  }
   ...
   * 从缓存获取模型列表
   */
  private getCachedModels(cacheKey: string, timeKey: string, ignoreExpiry = false): any[] | null {
    try {
