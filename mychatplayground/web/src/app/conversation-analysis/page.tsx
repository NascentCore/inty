...
请用清晰的结构化格式输出分析结果。`,
  ...
  {
    name: "长期记忆提取",
    prompt: `你是一个专业的用户画像分析师。请仔细阅读以下对话内容，从中提取关于用户的持久性信息，这些信息将作为AI助手的长期记忆，帮助AI更好地了解和服务用户。

请按以下类别提取信息，每条信息必须是明确的结论性陈述：
...
输出要求：
1. 每条记忆必须是独立的陈述句，以"用户"开头
2. 只提取有明确证据支持的信息，不要过度推测
3. 信息应具有时效性较长的特点（避免临时性信息）
4. 使用简洁明确的语言，每条不超过30字
5. 如果某个类别没有相关信息，标注"暂无"
6. 最后提供一段50-100字的用户画像总结
