...
"""
    ...
    async def _call_api(self, messages: List[Dict[str, str]], test_name: str) -> Dict[str, Any]:
        """调用API并记录结果"""
        ...
        try:
            ...
            print(f"   原始响应长度: {len(response_content) if response_content else 0}")
    ...
    def _analyze_results(self):
        ...
        if successful_tests:
            ...
            for test in successful_tests:
                ...
                print(f"   消息构成: System({system_count}) + User({user_count}) + Assistant({assistant_count}) = {len(messages)}")
                
                # 检查是否为有效响应
                if response and response.strip() and response != "[空响应]":
                    preview = response.strip()[:100] + "..." if len(response.strip()) > 100 else response.strip()
                    ...
                    # 检查是否符合角色设定
                    if any(keyword in response for keyword in ["daisies", "nature", "Lily"]):
        ...
        test_categories = {
            ...
            "多个System": ["multi_system", "multi_system_reordered", "multi_system_with_empty"],
            "历史对话测试": ["history_first", "minimal_with_history"]
        }
        
        for category, test_names in test_categories.items():
            category_tests = [r for r in successful_tests if r['test_name'] in test_names]
            valid_responses = [r for r in category_tests if r.get('response', '').strip() and r['response'] != "[空响应]"]
            name_mentioned = [r for r in valid_responses if "dx" in r.get('response', '').lower()]
            ...
            if category_tests:
                avg_time = sum(r['duration'] for r in category_tests) / len(category_tests)
