...
class AgentMigrator:
    ...
    def _parse_json_fields(self, agent_dict: Dict[str, Any]):
        """解析JSON字段"""
        json_fields = [
            'background_images', 'settings', 'photos', 'character_card_data',
            'alternate_greetings', 'character_book', 'tags', 'extensions'
        ]
    ...
    async def export_agents(self, from_env: str, output_file: Optional[str] = None) -> str:
        """从指定环境导出AI角色"""
        ...
        try:
            ...
            for row in rows:
                ...
                if export_config.get('include_creator_info', True):
                    ...
                    # 移除None值
                    creator_info = {k: v for k, v in creator_info.items() if v is not None}
                ...
                try:
                    ...
                    self.logger.warning(f"跳过无效角色数据 {agent_dict.get('id', 'unknown')}: {e}")
                    continue
            ...
            self.logger.info(f"成功导出 {len(agents_data)} 个AI角色到文件: {output_file}")
            return output_file
        ...
        try:
            ...
            for agent_data in agents_data:
                try:
                    ...
                    imported_count += 1
                    self.logger.debug(f"成功导入角色: {agent_data.get('name', 'unknown')}")
                except Exception as e:
                    error_count += 1
                    self.logger.error(f"导入角色失败 {agent_data.get('name', 'unknown')}: {e}")
                    if not import_config.get('skip_validation', False):
                        continue
            
            self.logger.info(f"导入完成: 成功 {imported_count}, 跳过 {skipped_count}, 错误 {error_count}")
            return imported_count
            
        finally:
            await conn.close()
    
    async def _import_single_agent(self, conn: asyncpg.Connection, agent_data: Dict[str, Any], import_config: Dict[str, Any]):
        """导入单个AI角色"""
        # 处理ID
        ...
        # 检查是否已存在
        existing = await conn.fetchrow("SELECT id FROM agents WHERE id = $1 OR readable_id = $2", 
                                     new_id, agent_data['readable_id'])
    ...
    async def _ensure_creator_exists(self, conn: asyncpg.Connection, agent_data: Dict[str, Any], 
                                   creator_id: Optional[str], import_config: Dict[str, Any]) -> Optional[str]:
        """确保创建者存在，如果不存在则创建或使用默认值"""
        if not creator_id:
            self.logger.warning(f"角色 {agent_data.get('name', 'unknown')} 没有创建者ID，将设置为NULL")
            return None
        
        # 检查创建者是否存在
        existing_user = await conn.fetchrow("SELECT id FROM users WHERE id = $1", creator_id)
        if existing_user:
            return creator_id
        ...
        if default_creator:
            # 检查默认创建者是否存在
            default_exists = await conn.fetchrow("SELECT id FROM users WHERE id = $1", default_creator)
            if default_exists:
                self.logger.warning(f"使用默认创建者 {default_creator} 替代不存在的创建者 {creator_id}")
                return default_creator
        
        self.logger.warning(f"创建者 {creator_id} 不存在且无法创建，将设置为NULL")
        return None
    
    async def _create_user(self, conn: asyncpg.Connection, user_id: str, creator_info: Dict[str, Any]):
        """创建用户"""
    ...
    async def _insert_agent(self, conn: asyncpg.Connection, agent_data: Dict[str, Any], 
                           import_config: Dict[str, Any], new_id: str):
        """插入新的AI角色"""
        # 处理创建者ID
        ...
        # 确保创建者存在
        creator_id = await self._ensure_creator_exists(conn, agent_data, creator_id, import_config)
        
        # 如果指定了强制状态
        status = import_config.get('force_status') or agent_data['status']
        
        # 移除不需要的字段
        agent_data_clean = {k: v for k, v in agent_data.items() 
        ...
        for i, (col, val) in enumerate(zip(columns, values)):
            if col in ['background_images', 'settings', 'photos', 'character_card_data', 
                      'alternate_greetings', 'character_book', 'tags', 'extensions'] and val:
    ...
    async def _update_agent(self, conn: asyncpg.Connection, agent_data: Dict[str, Any], 
                           import_config: Dict[str, Any], agent_id: str):
        """更新现有AI角色"""
        # 构建更新语句
        update_fields = []
        values = []
        ...
        for key, value in agent_data.items():
            ...
            # 处理JSON字段
            if key in ['background_images', 'settings', 'photos', 'character_card_data', 
                      'alternate_greetings', 'character_book', 'tags', 'extensions']:
...
async def main():
    ...
    parser.add_argument('--config', '-c', default='agent_migration_config.yaml', help='配置文件路径')
