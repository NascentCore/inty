...
class GooglePlayVerifier:
    ...
    def _test_single_subscription(self, product_id: str, purchase_token: str) -> None:
        ...
        try:
            ...
            if result.get("startTimeMillis"):
                start_time = datetime.fromtimestamp(int(result.get("startTimeMillis")) / 1000, timezone.utc)
                print(f"         📅 Start Time: {start_time}")
            
            if result.get("expiryTimeMillis"):
                expiry_time = datetime.fromtimestamp(int(result.get("expiryTimeMillis")) / 1000, timezone.utc)
    ...
    def _test_single_inapp_product(self, product_id: str, purchase_token: str) -> None:
        ...
        try:
            ...
            if result.get("purchaseTimeMillis"):
                purchase_time = datetime.fromtimestamp(int(result.get("purchaseTimeMillis")) / 1000, timezone.utc)
                print(f"         📅 Purchase Time: {purchase_time}")
            
            purchase_state = result.get("purchaseState", 0)
            state_names = {0: "Purchased", 1: "Canceled", 2: "Pending"}
            print(f"         💳 Purchase State: {state_names.get(purchase_state, purchase_state)}")
            
            consumption_state = result.get("consumptionState", 0)
            consumption_names = {0: "Yet to be consumed", 1: "Consumed"}
            print(f"         🍽️  Consumption State: {consumption_names.get(consumption_state, consumption_state)}")
            
            acknowledgement_state = result.get("acknowledgementState", 0)
            ack_names = {0: "Yet to be acknowledged", 1: "Acknowledged"}
            print(f"         ✅ Acknowledgement State: {ack_names.get(acknowledgement_state, acknowledgement_state)}")
    ...
    def _get_subscription_products(self) -> None:
        ...
        try:
            ...
            result = self.service.monetization().subscriptions().list(
                packageName=self.package_name
            ).execute()
            ...
            print(f"      ❌ Failed to retrieve subscription products: {error_details['message']}")
    ...
    def _get_inapp_products(self) -> None:
        ...
        try:
            ...
            print(f"      ❌ Failed to retrieve in-app products: {error_details['message']}")
            ...
            if error.content:
                ...
                if "error" in error_content:
                    ...
                    if error_info["errors"]:
                        ...
                        error_info["message"] = first_error.get("message", error_info["message"])
    ...
    def _generate_diagnostics(self) -> None:
        ...
        diagnostics["overall_health"] = {
            ...
            "subscription_operations": "✅ HEALTHY" if not has_subscription_issues else "❌ ISSUES",
            "inapp_product_operations": "✅ HEALTHY" if not has_inapp_issues else "❌ ISSUES",
        ...
        if has_subscription_issues:
            next_steps.append("Verify subscription configuration in Google Play Console")
        
        if has_inapp_issues:
            next_steps.append("Verify in-app product configuration in Google Play Console")
        
        if has_catalog_issues:
            next_steps.append("Fix product catalog retrieval permissions in Google Play Console")
        
        if not any([has_config_issues, has_auth_issues, has_permission_issues, has_subscription_issues, has_inapp_issues, has_catalog_issues]):
            next_steps.append("All tests passed! Your Google Play integration should be working correctly.")
...
def main():
    ...
    has_critical_failures = any([
        any(result.get("status") == "❌ FAILED" for result in results.get("config_validation", {}).values()),
        any(result.get("status") == "❌ FAILED" for result in results.get("authentication", {}).values())
    ])
