...
if ! command -v gh &> /dev/null; then
    ...
    echo "  Linux:   sudo apt install gh  (或 sudo dnf install gh)"
...
echo "📝 关闭 Issue #1360: Room 本地数据库集成..."
gh issue close 1360 -R "$REPO" --comment "此功能已完整实现，包含：
...
echo "📝 关闭 Issue #1691: inty setting 使用 datastore..."
gh issue close 1691 -R "$REPO" --comment "此功能已实现，使用了性能更优的 **MMKV** 替代方案（腾讯开源，性能优于 DataStore 约 100 倍）。
...
# Issue #582 - 角色标签功能
echo "📝 关闭 Issue #582: 角色提供内部标签功能..."
gh issue close 582 -R "$REPO" --comment "角色标签功能已完整实现。
...
# Issue #771 - AI 主动消息
echo "📝 关闭 Issue #771: AI 角色主动向用户发送消息..."
gh issue close 771 -R "$REPO" --comment "AI 角色主动消息功能已完整实现并在生产环境稳定运行。
...
echo "✅ Issue #771 已关闭"
echo ""
echo "🎉 成功！所有 4 个 Issues 已关闭。"
echo ""
echo "查看结果："
echo "  https://github.com/$REPO/issues/1360"
echo "  https://github.com/$REPO/issues/1691"
echo "  https://github.com/$REPO/issues/582"
echo "  https://github.com/$REPO/issues/771"
