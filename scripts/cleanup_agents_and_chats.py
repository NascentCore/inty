...
class AgentChatCleanup:
    ...
    async def cleanup_all_agents_and_chats(self) -> bool:
        ...
        try:
            ...
            # 验证清理结果
            if (after_counts['messages'] == 0 and 
                after_counts['chat_settings'] == 0 and
                after_counts['chats'] == 0 and 
                after_counts['agent_followers'] == 0 and 
                after_counts['agents'] == 0):
                logger.success("✅ 所有agent和chat数据已成功清理！")
                logger.info(f"✅ 保留用户数据: {after_counts['users']} 个用户")
                logger.info(f"✅ 保留订阅计划: {after_counts['subscription_plans']} 个计划")
                logger.info(f"✅ 保留用户订阅: {after_counts['user_subscriptions']} 个订阅")
                logger.info(f"✅ 清理了举报记录: {before_counts.get('report', 0)} 条")
                logger.info(f"✅ 清理了通知记录: {before_counts.get('user_notifications', 0)} 条")
                return True
            else:
                logger.error("❌ 数据清理可能不完整，请检查！")
                logger.error(f"剩余数据: messages={after_counts['messages']}, chat_settings={after_counts['chat_settings']}, chats={after_counts['chats']}, agent_followers={after_counts['agent_followers']}, agents={after_counts['agents']}")
                return False
    ...
    async def run(self):
        ...
        async with AsyncSessionLocal() as session:
            ...
            try:
                ...
                # 确认操作
                print(f"\n")
                confirmation = input("请输入 'DELETE' 来确认删除操作（其他任何输入都会取消）: ")
                ...
                # 二次确认
                print(f"\n")
                final_confirmation = input("最后确认：此操作不可逆！请再次输入 'DELETE' 确认: ")
