...
class CharacterAgent:
    ...
    def generate_character(
        ...
        try:
            ...
            self.logger.info(
                f"Character generation completed successfully in {generation_time:.2f} seconds"
            )

            return CharacterGenerationResponse(
                success=True, character=character, generation_time=generation_time
            )

        except Exception as e:
            generation_time = time.time() - start_time
            self.logger.error(
                f"Character generation failed after {generation_time:.2f} seconds: {str(e)}"
            )
            self.logger.exception("Full exception details:")

            return CharacterGenerationResponse(
                success=False, error=str(e), generation_time=generation_time
            )
    ...
    def _validate_character(self, character: CharacterProfile) -> None:
        ...
        for field_name, value in required_fields:
            if not value or (isinstance(value, str) and value.strip() == ""):
                ...
                raise ValueError(
                    f"Character profile is missing required information: {field_name}"
                )
            else:
        ...
        if not appearance.get("hair_color") or not appearance.get("eye_color"):
            ...
            self.logger.debug(
                f"✅ Physical appearance complete: hair={appearance.get('hair_color')}, eyes={appearance.get('eye_color')}"
            )
        ...
        if not character.encounter.initial_dialogue:
            ...
            self.logger.debug(
                f"✅ Encounter dialogue present: {character.encounter.initial_dialogue[:50]}..."
            )
    ...
    def get_character_summary(self, character: CharacterProfile) -> dict:
        ...
        summary = {
            ...
            "main_motivation": (
                character.background.motivations[0]
        ...
        self.logger.debug(
            f"Summary generated: {summary['name']} ({summary['age']} years old)"
        )
        return summary

    def export_character(
        self, character: CharacterProfile, format: str = "json"
    ) -> str:
