...
class TestTagParser(unittest.TestCase):
    ...
    def test_extract_tags_double_quotes(self):
        """测试双引号JSON格式"""
        result = self.parser.extract_tags_from_personality(
            SAMPLE_PERSONALITIES[2], 
            "test-agent-2", 
            "Test Agent 2"
        )
        
        self.assertTrue(result.extraction_success)
        self.assertEqual(result.extracted_tags, ['Creative', 'Artistic', 'Passionate', 'Talented'])
    
    def test_extract_tags_mixed_quotes(self):
        """测试混合引号格式"""
        result = self.parser.extract_tags_from_personality(
            SAMPLE_PERSONALITIES[3], 
            "test-agent-3", 
            "Test Agent 3"
        )
        
        self.assertTrue(result.extraction_success)
        self.assertEqual(result.extracted_tags, ['Teacher', 'Kind', 'Patient', 'Caring'])
    
    def test_extract_tags_no_character_info(self):
        """测试没有Character info的情况"""
        result = self.parser.extract_tags_from_personality(
            SAMPLE_PERSONALITIES[11], 
            "test-agent-4", 
            "Test Agent 4"
        )
    ...
    def test_validate_tags(self):
        """测试标签验证"""
        tags = ['ValidTag', 'a', '123', 'Normal-Tag', 'Tag With Spaces', 'Too@Many!Special#Chars']
    ...
    def test_comprehensive_extraction(self):
        """综合测试：测试所有样本数据"""
        for i, (personality, expected) in enumerate(zip(SAMPLE_PERSONALITIES, EXPECTED_RESULTS)):
            with self.subTest(i=i):
                ...
                if expected:  # 如果预期有结果
                    ...
                    self.assertFalse(result.extraction_success or result.extracted_tags,
                                   f"样本 {i} 不应该提取成功")
