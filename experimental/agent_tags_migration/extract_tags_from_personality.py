...
class TagMigrator:
    ...
    async def initialize(self):
        ...
        self.logger = setup_logger(
            level=self.migration_config.log_level,
            log_file=log_file
        )
    ...
    async def migrate_tags(self) -> MigrationStats:
        ...
        while offset < total_count:
            ...
            if self.migration_config.backup_before_update:
                backup_data = await self.db_manager.backup_agents_data([a.id for a in agents])
    ...
    async def _process_batch_analysis(self, agents) -> List[TagExtractionResult]:
        ...
        for agent in agents:
            ...
            # 检查是否需要跳过已有标签的智能体
            if (self.migration_config.skip_existing_tags and 
                agent.tags and len(agent.tags) > 0):
                result.error_message = "智能体已有标签，跳过处理"
                self.migration_logger.log_extraction_failure(agent.name, result.error_message)
            elif result.extraction_success:
                # 标准化标签
                if self.migration_config.normalize_tags:
                    result.extracted_tags = self.tag_parser.normalize_tags(result.extracted_tags)
                
                # 验证标签
                valid_tags, invalid_tags = self.tag_parser.validate_tags(result.extracted_tags)
                if invalid_tags:
    ...
    async def _process_batch_migration(self, agents) -> List[TagExtractionResult]:
        ...
        for result in results:
            if (result.extraction_success and 
                result.extracted_tags and 
                not (result.error_message and result.error_message.startswith("智能体已有标签"))):
        ...
        if updates:
            ...
            for i, (agent_id, tags) in enumerate(updates):
                ...
                if i < success_count:
                    ...
                    self.migration_logger.log_update_failure(agent_name, "数据库更新失败")
    ...
    def _generate_stats(self, results: List[TagExtractionResult]) -> MigrationStats:
        ...
        # 统计可提取标签的智能体
        stats.agents_with_extractable_tags = len([r for r in results if r.character_info and r.character_info.tags])
        
        # 统计更新成功的智能体
        stats.updated_agents = len([r for r in successful_results if r.extracted_tags])
        
        # 统计跳过的智能体
        stats.skipped_agents = len([r for r in results if r.error_message and "跳过" in r.error_message])
...
@click.option('--config', '-c', default='../../config.yaml', 
              help='配置文件路径', type=click.Path(exists=True))
@click.option('--analyze', is_flag=True, 
              help='分析模式：只分析数据，不进行更新')
...
def main(config, analyze, update, batch_size, skip_existing, normalize, log_level, log_file):
    """
...
async def run_migration(config_path: str, migration_config: MigrationConfig, analyze_only: bool):
    """运行迁移流程"""
