...
# 匹配标签的正则表达式
    ...
    def extract_tags_from_personality(self, personality: str, agent_id: str, agent_name: str) -> TagExtractionResult:
        """
        从personality字段中提取标签
...
#从角色信息中提取标签
                if character_info.tags:
                    ...
                    if tags:
                        ...
                        result.error_message = f"无法解析tags字符串: {character_info.tags}"
                else:
    ...
    def _extract_character_info(self, text: str) -> Tuple[Optional[CharacterInfo], Optional[str]]:
        """
        提取Character info结构
...
# 不能全是数字
    ...
    def get_stats_from_results(self, results: List[TagExtractionResult]) -> Dict[str, Any]:
        """
        从提取结果中生成统计信息
