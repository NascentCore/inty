...
class DatabaseManager:
    ...
    async def get_agents_with_personality(self, limit: Optional[int] = None, offset: int = 0) -> List[Agent]:
        """
    ...
    async def update_agent_tags(self, agent_id: str, tags: List[str]) -> bool:
        ...
        try:
            async with self.get_connection() as conn:
                ...
                result = await conn.execute(query, tags_json, datetime.utcnow(), agent_id)
                return result == "UPDATE 1"
        except Exception:
            return False
    
    async def batch_update_agent_tags(self, updates: List[Tuple[str, List[str]]]) -> Tuple[int, int]:
        """
        批量更新智能体tags
