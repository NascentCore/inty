...
class CompactionConfig(BaseModel):
    ...
    max_episodic_entries: int = Field(gt=5)
    max_semantic_entries: int = Field(gt=5)
...
class ConversationCompactor:
    ...
    def __init__(self, config: CompactionConfig) -> None:
        self._config = config
        self._state = CompactionState.empty()
    ...
    def _build_compaction_system_prompt(
        ...
        return "\n".join(
            [
                ...
                f"turn={turn}",
                "你看到的是历史对话压缩后的长期记忆快照，优先服从用户最新输入；若冲突，以最新输入为准。",
                "## Running Summary",
...
def _build_episode_summary(
    *, user_texts: list[str], assistant_texts: list[str]
    ...
    if user_preview and assistant_preview:
        return f"用户提出: {user_preview} | 角色回应: {assistant_preview}"
    if user_preview:
        return f"用户提出: {user_preview}"
    if assistant_preview:
        return f"角色回应: {assistant_preview}"
    return "空白片段"
