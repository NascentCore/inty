...
- **REPL 上下线**：`main.repl` 在进入交互循环前追加一行 `role=user` 且 **`presence`: `repl_online`**，退出（含 quit / EOF / 正常离开循环）后追加 **`presence`: `repl_offline`**；`content` 为固定中文系统提示。该类行**不算**真实用户输入，陪伴心跳调度会忽略末尾仅含 `presence` 的 user 行（见 `models.transcript_without_trailing_presence_signals`）。
