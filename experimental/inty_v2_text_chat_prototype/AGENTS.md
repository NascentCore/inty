...
- REPL **陪伴心跳**（见 `main.repl` / `heartbeat_schedule.py`）：空闲达到节奏阈值时由程序合成一轮 `user` 行（`content` 为 `HEARTBEAT_SYNTHETIC_USER_TEXT`：要求读本窗口场景与语气**自然续接**，勿改换风格），可带 **`heartbeat`: true**；`build_system_prompt` 的「本轮（陪伴心跳）」段与此一致。该回合不跑记忆管线，且 API 不挂载工具。`heartbeat_schedule.next_heartbeat_wait_seconds(..., heartbeat_enabled=...)` 与 `--repl-heartbeat` / `--no-repl-heartbeat` 对齐。**未设置** `INTY_V2_PROTO_HEARTBEAT` 时默认**开启**空闲心跳；显式关闭：`INTY_V2_PROTO_HEARTBEAT=0`（或 `false`/`off`）或 `--no-repl-heartbeat`。另：`INTY_V2_PROTO_HEARTBEAT_MIN_GAP_SEC` 默认较大（与上次**陪伴心跳** user 行间隔），若久未出现心跳可先检查是否未到间隔。
