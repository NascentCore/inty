...
- **eval_prod_failed_prompts.py**：对 `tmp/scene_prompt_*.txt` 逐条读 prompt，调用 fal 生成并落盘；默认 text-to-image。传入 `--image-url` 时改为 image-to-image；传入 `--image-path` 时会先将该本地文件上传到 fal CDN，再用返回的 URL 做 image-to-image（无需自备公网 URL）。若某条 prompt 触发 fal 报错（如 422 content_policy_violation）：会打印异常、将完整错误信息写入 `{output_dir}/{files_prefix}_fal_output_{timestamp}_error.json`，然后继续下一条。示例：`python -m experimental.eval_nana_banana.fal.eval_prod_failed_prompts --model fal-ai/gpt-image-1.5/edit --image-path tests/files/nurse_char_full_body.jpeg`。
