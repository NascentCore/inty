...
def analyze_sentiment_demo(message: str) -> str:
    ...
    if any(word in message_lower for word in ["开心", "高兴", "快乐", "兴奋", "太棒了", "awesome", "great"]):
        return "happy"
    elif any(word in message_lower for word in ["难过", "伤心", "沮丧", "失望", "sad", "upset"]):
        return "sad"
    elif any(word in message_lower for word in ["生气", "愤怒", "恼火", "angry", "mad"]):
        return "angry"
    elif any(word in message_lower for word in ["惊讶", "震惊", "意外", "surprised", "wow"]):
        return "surprised"
    elif any(word in message_lower for word in ["担心", "焦虑", "紧张", "worried", "anxious"]):
        return "worried"
...
@app.get("/")
async def root():
    return {"message": "Chatbot with Image Selection Demo API (Demo Mode - No Gemini API required)"}
