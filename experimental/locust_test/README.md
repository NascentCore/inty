...
本目录包含用于 Inty 后端聊天接口并发性能测试的完整解决方案，使用 Locust 框架进行负载测试，支持在 Google Cloud Platform 上部署和运行。
...
- **主要目标**：评估 Inty 后端在 1c2g 资源限制下的并发性能
- **测试环境**：GCP VM (4c8g) + Docker Container (1c2g 限制)
- **核心接口**：
  - 游客注册:`POST /api/v1/auth/guest`- 代理聊天：`POST /api/v1/chats/agents/{agent_id}/chat/completions`
...
步骤详细请参考 [gcp-deployment.md](./gcp-deployment.md)医学博士）```bash
# 创建GCP VM
...
- **config.test.yaml**：基于生产环境config.yaml的测试配置
- **inty-backend-key.json**: Google Cloud 服务账号密钥（测试环境使用模拟文件）
- **inty-firebase-key.json**: Firebase 服务账号密钥（测试环境使用模拟文件）

### 测试参数

- **并发用户**：可通过Locust Web界面或命令行调整
...
1.启动Locust服务：`docker-compose -f docker-compose.test.yml up -d locust-master`2. 访问：`http://localhost:8089`
3.设置参数并开始测试
...
- **Locust 仪表板**：`http://localhost:8089`
