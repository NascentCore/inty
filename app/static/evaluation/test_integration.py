...
async def test_existing_apis():
    ...
    async with APITester(BASE_URL) as tester:
        ...
        print(f"  ✓ 获取智能体列表: {len(agents) if isinstance(agents, list) else 'Error'}")
        ...
        if agents and isinstance(agents, list) and len(agents) > 0:
            ...
            if agent_id:
                ...
                try:
                    chat_response = await tester.post(f"/chats/agents/{agent_id}/chat/completions", message_data)
    ...
    async with APITester(BASE_URL) as tester:
        ...
        if agents and isinstance(agents, list) and len(agents) > 0:
            ...
            if session and session.get('id'):
                ...
                # 启动评测
                start_result = await tester.post(f"/evaluation/sessions/{session_id}/start")
