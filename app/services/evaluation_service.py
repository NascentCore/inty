...
class EvaluationService:
    ...
    async def create_session(
        ...
        logger.info(f"创建评测会话: {session.id}, 问题数: {len(questions)}, 智能体数: {len(selected_agents)}")
        return session
    ...
    async def _execute_session(self, session_id: str):
        ...
        async with AsyncSessionLocal() as db_session:
            ...
            try:
                ...
                # 广播开始消息
                await bg_service._broadcast_update(session.id, {
                ...
                for question_index, question in enumerate(session.questions):
                    ...
                    for agent_id in session.selected_agents:
                        ...
                        await bg_service._broadcast_update(session.id, {
                            ...
                            'question': question,
                            'agent_id': agent_id,
                            'progress': int((test_index / session.total_tests) * 100)
                        })
                    ...
                    # 并行执行当前问题的所有智能体测试
                    logger.info(f"并行执行问题 {question_index + 1} 的 {len(agent_tasks)} 个智能体测试")
                    ...
                    for i, result in enumerate(results):
                        ...
                        if isinstance(result, Exception):
                            logger.error(f"测试执行异常 {current_test_index}: {str(result)}")
                            continue
                            
                        if result:
                            session_state['results'].append(result)
                            session_state['current_test'] = current_test_index
                            session_state['progress'] = int((current_test_index / session.total_tests) * 100)
                    ...
                    logger.info(f"问题 {question_index + 1} 的并行测试完成，成功 {len([r for r in results if not isinstance(r, Exception) and r])} 个")
    ...
    async def _prepare_user_identity(self, session: EvaluationSession) -> Dict[str, Any]:
        """准备用户身份信息"""
    ...
    async def get_session(self, session_id: str) -> Optional[EvaluationSession]:
        """获取评测会话"""
        stmt = select(EvaluationSession).options(
    ...
    async def get_user_sessions(
        ...
        stmt = stmt.order_by(EvaluationSession.created_at.desc()).offset(skip).limit(limit)
    ...
    async def _send_evaluation_message(
        ...
        try:
            ...
            # 获取智能体数据（用于创建Agent实例）
            agent_data = await agent_service.get_agent_for_chat(self.db, agent_id=agent_id)
            if not agent_data:
