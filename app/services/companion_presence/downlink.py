"""Channel-agnostic downlink events from ``CompanionPresence`` to a transport adapter.

WebSocket and Weixin adapters translate :class:`CompanionDownlink` into
...
class CompanionDownlinkKind(StrEnum):
...
class CompanionChannelDownlink(Protocol):
    """Transport adapter: materialize and send one :class:`CompanionDownlink`."""

    async def deliver(self, event: CompanionDownlink) -> None:
        """Deliver ``event`` on this channel (WS queue pump, Weixin peer text, etc.)."""


def user_reply_downlink(*, turn: CompanionTurnResult) -> CompanionDownlink:
    """Foreground user-chat (or implicit greeting) turn finished."""
    assert turn is not None
    return CompanionDownlink(
        kind=CompanionDownlinkKind.USER_REPLY,
...
def proactive_downlink(
    *,
    ...
    return CompanionDownlink(
        kind=CompanionDownlinkKind.PROACTIVE,
        assistant_text=turn.assistant_text,
        turn=turn,
        tool_output=None,
        bootstrap_interim=None,
...
def maintenance_downlink(
    *,
    ...
    assert transcript_user_text.strip()
    return CompanionDownlink(
        kind=CompanionDownlinkKind.MAINTENANCE,
        assistant_text=turn.assistant_text,
        turn=turn,
        tool_output=None,
        bootstrap_interim=None,
...
def scheduled_downlink(
    *,
    ...
    return CompanionDownlink(
        kind=CompanionDownlinkKind.SCHEDULED,
        assistant_text=turn.assistant_text,
        turn=turn,
        tool_output=None,
        bootstrap_interim=None,
...
def tool_background_downlink(*, tool_output: ToolOutputEvent) -> CompanionDownlink:
    """Async tool loop produced user-visible text (may be suppressed by adapter flags)."""
    assert tool_output is not None
    return CompanionDownlink(
        kind=CompanionDownlinkKind.TOOL_BACKGROUND,
...
def bootstrap_interim_downlink(
    *,
    ...
    return CompanionDownlink(
        kind=CompanionDownlinkKind.BOOTSTRAP_INTERIM,
...
def downlink_delivers_user_visible_text(event: CompanionDownlink) -> bool:
    """Whether adapters should push assistant text to the human on this channel."""
    match event.kind:
        case CompanionDownlinkKind.TOOL_BACKGROUND:
            assert event.tool_output is not None
            return bool(event.tool_output.output_to_user) and bool(
                event.assistant_text.strip()
            )
        case CompanionDownlinkKind.MAINTENANCE:
            return bool(event.assistant_text.strip())
        case (
            CompanionDownlinkKind.USER_REPLY
            | CompanionDownlinkKind.PROACTIVE
            | CompanionDownlinkKind.SCHEDULED
            | CompanionDownlinkKind.BOOTSTRAP_INTERIM
        ):
            return bool(event.assistant_text.strip())
        case _:
            return False
