...
# Feedback 推送的聊天轮数阈值
FEEDBACK_PUSH_THRESHOLDS = [20, 30, 40, 50, 60]
...
async def has_user_submitted_feedback(db: AsyncSession, user_id: str) -> bool:
    ...
    try:
        ...
        logger.error(f"检查用户是否已填写 feedback 失败: user_id={user_id}, error={str(e)}")
        return False


async def should_send_feedback_push(
    db: AsyncSession,
    user_id: str,
    ...
    """
    判断是否应该发送 feedback 推送

    Args:
        db: 数据库会话
        user_id: 用户ID
        total_chat_count: 用户总聊天轮数
        threshold: 触发的阈值
    ...
    try:
        # 1. 检查是否达到阈值
        if total_chat_count < threshold:
            return False

        # 2. 检查是否已填写 feedback
        if await has_user_submitted_feedback(db, user_id):
            logger.debug(
                f"用户已填写 feedback，跳过推送: user_id={user_id}, threshold={threshold}"
            )
            return False

        # 3. 检查是否已为该阈值发送过（通过唯一约束保证，但这里先检查避免重复尝试）
        existing = await db.execute(
            select(FeedbackPushHistory).where(
                and_(
                    FeedbackPushHistory.user_id == user_id,
                    FeedbackPushHistory.chat_count_threshold == threshold,
                )
            )
        )
        if existing.scalar_one_or_none():
            logger.debug(
                f"用户已为该阈值发送过推送，跳过: user_id={user_id}, threshold={threshold}"
            )
            return False
        ...
        recent_push = await db.execute(
            select(FeedbackPushHistory).where(
                and_(
                    FeedbackPushHistory.user_id == user_id,
                    FeedbackPushHistory.sent_at >= twenty_four_hours_ago,
                )
            )
        )
        if recent_push.scalar_one_or_none():
            logger.debug(
                f"用户 24 小时内已发送过推送，跳过: user_id={user_id}, threshold={threshold}"
            )
            return False
        ...
        logger.error(
            f"判断是否应该发送 feedback 推送失败: user_id={user_id}, threshold={threshold}, error={str(e)}"
        )
        return False


async def send_feedback_push(
    db: AsyncSession,
    user_id: str,
    threshold: int,
) -> bool:
    ...
    try:
        ...
        if success:
            # 记录推送历史
            sent_at = datetime.datetime.now(datetime.timezone.utc)
            push_history = FeedbackPushHistory(
                id=uid("fpush"),
                user_id=user_id,
            ...
            logger.info(
                f"Feedback 推送发送成功: user_id={user_id}, threshold={threshold}"
            )
            return True
        else:
            logger.warning(
                f"Feedback 推送发送失败: user_id={user_id}, threshold={threshold}"
            )
            return False

    except Exception as e:
        logger.error(
            f"发送 feedback 推送失败: user_id={user_id}, threshold={threshold}, error={str(e)}"
        )
        import traceback
...
async def process_feedback_push_batch(
    db: AsyncSession,
    batch_size: int = 50,
) -> Tuple[int, int]:
    """
    ...
    try:
        logger.info("开始批量处理 feedback 推送")

        # 查询所有活跃用户（有 device token 且未删除）
        users_stmt = (
            select(User.id)
            .where(User.deleted_at.is_(None))
            .limit(batch_size * 10)  # 查询更多用户，因为很多会被过滤
        )
        users_result = await db.execute(users_stmt)
        user_ids = [row[0] for row in users_result.all()]

        logger.info(f"找到 {len(user_ids)} 个活跃用户，开始检查是否需要推送")

        # 对每个用户检查是否应该发送
        for user_id in user_ids:
            try:
                # 检查用户是否有 device token
                if not await _check_user_has_device_token(db, user_id):
                    continue
                ...
                # 检查是否达到任何阈值
                for threshold in FEEDBACK_PUSH_THRESHOLDS:
                    if await should_send_feedback_push(
                        db, user_id, total_chat_count, threshold
                    ):
                        # 发送推送
                        success = await send_feedback_push(db, user_id, threshold)
                        if success:
                            success_count += 1
                            logger.info(
                                f"Feedback 推送成功: user_id={user_id}, threshold={threshold}, total_chat_count={total_chat_count}"
                            )
                        else:
                            fail_count += 1
                            logger.warning(
                                f"Feedback 推送失败: user_id={user_id}, threshold={threshold}"
                            )
                        # 每个用户每次只发送一次（即使达到多个阈值，也只发送第一个）
                        break
                ...
                logger.error(
                    f"处理用户 feedback 推送时出错: user_id={user_id}, error={str(e)}"
                )
                fail_count += 1
                continue

        logger.info(
            f"Feedback 推送批量处理完成: 成功={success_count}, 失败={fail_count}"
        )
        return success_count, fail_count

    except Exception as e:
        logger.error(f"批量处理 feedback 推送失败: {str(e)}")
        import traceback
