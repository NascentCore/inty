...
class KeepTalkingService:
    def __init__(
        ...
        """
        self.check_interval = check_interval if check_interval is not None else settings.keep_talking.check_interval
        self.max_idle_time = max_idle_time if max_idle_time is not None else settings.keep_talking.max_idle_time
        self.max_keep_talking_messages = max_keep_talking_messages if max_keep_talking_messages is not None else settings.keep_talking.max_keep_talking_messages
        self._running = False
        self._task = None
            ...
            try:
                ...
                result = await db.execute(
                    ...
                    .where(
                        and_(
                            Chat.is_active == True,
                            ChatSettings.keep_talking == True
                        )
                    )
                )
                ...
                for chat in chats:
                    try:
                        ...
                        try:
                            ...
                            logger.warning(f"会话 {chat.id} 时区转换失败: {str(tz_e)}, 跳过此会话")
                            continue
                ...
                for chat in idle_chats:
                    try:
                        ...
                        logger.error(f"为会话 {chat.id} 发送keep_talking消息时出错: {str(e)}")
    ...
    async def _send_keep_talking_message(self, chat: Chat):
        ...
        try:
            ...
            # 构建keep_talking提示词
            keep_talking_prompt = self._generate_keep_talking_prompt(messages_context, chat.agent.name)
            
            # 构建消息格式
            messages = {
                "messages": [HumanMessage(content=keep_talking_prompt)]
            }
            ...
            # 更新keep_talking计数
            self._keep_talking_counts[chat.id] = self._keep_talking_counts.get(chat.id, 0) + 1
            
            logger.info(f"成功为会话 {chat.id} 发送keep_talking消息: {response[:100]}...")
