...
class CharacterCardService:
    ...
    async def import_character_card(
        ...
        try:
            ...
            if not request.override_existing:
                existing_agent = await self._check_existing_agent(card_data.data.name, user_id, db)
                if existing_agent:
                    return CharacterCardImportResponse(
                        success=False,
            ...
            # 处理可选功能
            imported_features = ['basic_info', 'personality', 'scenario', 'system_prompt']
            warnings = []
            ...
            if not request.import_alternate_greetings and agent_data.get('alternate_greetings'):
            ...
            logger.info(f"成功导入角色卡: {card_data.data.name} -> Agent ID: {agent.id}")
    ...
    async def import_character_card_from_file(
        ...
        try:
            ...
            if file.filename.lower().endswith('.json'):
                ...
                return CharacterCardImportResponse(
                    success=False,
                    message="不支持的文件格式，请上传JSON或PNG文件"
                )
            
            if not card_data:
                return CharacterCardImportResponse(
                    success=False,
                    message="无法从文件中解析角色卡数据"
                )
    ...
    async def export_agent_to_character_card(
        ...
        agent_data = {
            ...
            'alternate_greetings': agent.alternate_greetings if include_alternate_greetings else [],
    ...
    async def validate_character_card(
        ...
        try:
            ...
            try:
                ...
                errors.append(CharacterCardValidationError(
                    field="schema",
