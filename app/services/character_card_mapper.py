...
class CharacterCardMapper:
    ...
    def __init__(self):
        self.gender_keywords = {
            'male': ['man', 'boy', 'male', 'he', 'him', 'his', 'gentleman', 'guy', 'dude'],
            'female': ['woman', 'girl', 'female', 'she', 'her', 'hers', 'lady', 'gal'],
            'other': ['non-binary', 'they', 'them', 'their', 'enby', 'genderfluid']
        }
    
    def map_card_to_agent(self, card_data: CharacterCardV2, user_id: str) -> Dict[str, Any]:
        """
        将角色卡数据映射到Agent字段
        ...
        agent_data = {
            ...
            'opening': data.first_mes or data.alternate_greetings[0] if data.alternate_greetings else "",
            ...
            'character_book': data.character_book.dict() if data.character_book else None,
            'tags': data.tags,
    ...
    def map_agent_to_card(self, agent_data: Dict[str, Any]) -> CharacterCardV2:
        ...
        card_data = CharacterCardDataV2(
            ...
            creator=agent_data.get('creator', {}).get('nickname', '') if agent_data.get('creator') else '',
    ...
    def _build_system_prompt(self, data: CharacterCardDataV2) -> str:
        ...
        return "\n\n".join(prompt_parts) if prompt_parts else "你是一个AI助手，请用友好的方式回答用户的问题。"
    ...
    def _infer_gender(self, data: CharacterCardDataV2) -> Gender:
        ...
        for gender, keywords in self.gender_keywords.items():
            for keyword in keywords:
                gender_scores[gender] += len(re.findall(r'\b' + keyword + r'\b', text_to_analyze))
