...
class AsyncVoiceService:
    ...
    async def _generate_voice_task(
        ...
        try:
            ...
            audio_url = await self.voice_service.generate_voice(
                text=text, voice_id=voice_id, language=language, db=db
            )
            ...
            if callback_url and audio_url:
                await self._send_callback_notification(callback_url, task_id, audio_url)
    ...
    async def _send_callback_notification(
        self, callback_url: str, task_id: str, audio_url: str
    ) -> None:
        """
        发送回调通知
        """
    ...
    async def check_cache_first(
        ...
        if db is None:
            ...
            return await self._check_cache_first_impl(db, text, voice_id, language)
    ...
    def get_task_stats(self) -> Dict[str, Any]:
        ...
        if completed_tasks:
            total_time = sum(
                task["completed_at"] - task["created_at"] for task in completed_tasks
            )
