...
class CharacterBook(BaseModel):
    ...
    extensions: Dict[str, Any] = Field(default_factory=dict, description="扩展数据")
    entries: List[CharacterBookEntry] = Field(
...
class CharacterCardDataV2(BaseModel):
    ...
    alternate_greetings: List[str] = Field(
        default_factory=list, description="替代问候语"
    )
...
class CharacterCardImportRequest(BaseModel):
    ...
    card_data: Union[CharacterCardV2, Dict[str, Any]] = Field(
        ..., description="角色卡数据"
    )
...
class CharacterCardImportResponse(BaseModel):
    ...
    imported_features: List[str] = Field(
        default_factory=list, description="导入的功能列表"
    )
...
class CharacterCardValidationResponse(BaseModel):
    ...
    errors: List[CharacterCardValidationError] = Field(
        default_factory=list, description="验证错误"
    )
    warnings: List[str] = Field(default_factory=list, description="警告信息")
