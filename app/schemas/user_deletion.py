...
class AccountDeletionResponse(BaseModel):
    ...
    anonymized_fields: Optional[List[str]] = Field(None, description="已匿名化的字段列表")
...
class UserDeletionLogSchema(BaseModel):
    ...
    original_user_data: Optional[Dict[str, Any]] = Field(None, description="原始用户数据快照")
    deletion_reason: Optional[str] = Field(None, description="删除原因")
    deletion_type: str = Field(..., description="删除类型")
    anonymized_fields: Optional[List[str]] = Field(None, description="已匿名化的字段列表")
    subscription_status_at_deletion: Optional[str] = Field(None, description="删除时订阅状态")
