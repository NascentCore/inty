...
>
> **`request.messageType`（companion WebSocket）**：可选，默认 `USER_MESSAGE`；`IMPLICIT_USER_SIGNED_ON` 表示隐式「用户上线」回合（用户可见正文须为空，且不得含图片等多模态 content），服务端不写用户 history 行；companion 在 transcript 后追加 tail `system` 触发问候（见 FR 文档）；HTTP completions 传入则 **400**。校验失败时下行 **`code` 422**，`message`=`Invalid chat WebSocket request`。`record_usage` 的 `extra_data` 含 `implicit_user_signed_on: true` 便于区分。详见 [`/docs/FR_USER_SIGN_ON_GREETINGS.md`](/docs/FR_USER_SIGN_ON_GREETINGS.md)。
