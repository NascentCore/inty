...
>
> **Companion proactive heartbeat（`/api/v1/chat/ws`）**：`user_signed_on` 的产品意图是让用户上线时陪伴智能体呈现「觉察对方出现并主动问候」的拟人效果（见 [`/docs/FR_USER_SIGN_ON_GREETINGS.md`](/docs/FR_USER_SIGN_ON_GREETINGS.md)）。`app.features.companion_ws_proactive_heartbeat_enabled` 默认为 true（可在 YAML 设为 false 关闭）。开启时，客户端须先发送 **控制帧** `{"type":"user_signed_on","agent_id":"<uuid>"}`（不经 `outbound_queue`；服务端直回 `{"type":"user_signed_on_ack","ok":true}` 或 `ok:false` 与 `reason`），以登记本连接上的 `user_id`/`agent_id`/`chat_id` 坐标，之后连接存活期间后台按 `app.features.companion_ws_proactive_heartbeat_poll_seconds`（默认 45）检查 companion workspace transcript；当 `next_heartbeat_wait_seconds`（`HeartbeatConfig(enabled=True)`）返回 `<= 0` 且订阅允许时，服务端自动跑一轮 `InnerTickMode.PROACTIVE_CHAT` 的 `run_turn`，并向队列推送与正常轮次同形的 assistant 下行（用户侧落库短占位文案，`meta_data` 含 `companion_proactive_heartbeat`、`heartbeat`）。切换当前会话 agent 时须重发 `user_signed_on`。`/api/v1/chat/ws/verify` 对 `user_signed_on` 回 `ok:false, reason:not_supported`。
>
