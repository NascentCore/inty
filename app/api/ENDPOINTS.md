...
>
> **Dual-LLM async（`/api/v1/chat/ws` production）**：每轮先返回前台 chat（`response_format` JSON envelope：`user_facing_reply` + significance），工具调用仅在后台线程跑完整 tool loop（无同步 tool loop）。工具侧产出可见文本或生图时，服务端可能在同一 WebSocket 连接上再 `send_json` 一条与正常轮次同形的 assistant 回复，并写入 `chat_history`（`meta_data.source`=`tool_bg`，含 `trace_id`、`reply_to_user_msg_uuid`；生图成功时另含 `meta_data.generated_image`，其中 `image_url` 为 `gs://...`）。前台 HTTP 超时：`INTY_V2_PROTO_ASYNC_CHAT_FRONT_TIMEOUT_SEC`（默认 600 秒）。
>
