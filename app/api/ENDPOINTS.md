...
>
> **Companion inner tick on `/api/v1/chat/ws`**（默认开启，无单独开关）：当该连接上最近一次成功完成的聊天回合走了 companion kernel（`chat_use_companion_kernel_agent_ids` 命中）时，服务端在「空闲等待」与「内在节拍」二者中取**较短**的 `asyncio.wait` 超时；仅当内在节拍等待 **小于** `chat_ws_idle_timeout_seconds` 时才会与 idle 竞合（否则 idle 先到期关连，避免长 inner 间隔占满等待）。到期且 transcript 满足条件时额外推送一条 JSON，顶层含 `companion_inner_tick: true`（老客户端应忽略未知 JSON 字段）。`data` 形状与正常聊天回合一致（assistant 文本可能为空）。该推送会写入 `chat_history`（用户行 `content` 可为空字符串，`meta_data.companionInnerTick: true`），并计入 `record_usage` 的 `chat`（`extra_data.companion_inner_tick: true`）；额度用尽时跳过 inner。可调：`companion_ws_inner_tick_min_gap_seconds`、`companion_ws_inner_tick_first_after_user_seconds`（首轮 inner 距上次用户 companion 回合的最小间隔）、`companion_ws_inner_tick_min_transcript_messages`、`companion_ws_inner_tick_poll_cap_seconds`、`companion_ws_inner_tick_blocked_max_seconds`。
