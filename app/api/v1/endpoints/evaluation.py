...
@router.get("/sessions/{session_id}/results", response_model=List[schemas.EvaluationResultResponse])
...
async def parse_questions_file(
    *,
    ...
    try:
        ...
        logger.info(f"用户 {current_user.id} 上传问题文件: {file.filename}, 解析出 {len(questions)} 个问题")
        ...
        if session:
            await websocket.send_json({
                ...
                "data": {
                    ...
                    "progress": session.completed_tests / session.total_tests * 100 if session.total_tests > 0 else 0
                }
            })
...
async def get_evaluation_agents(
    *,
    ...
    try:
        ...
        logger.info(f"用户 {current_user.id} 获取评测智能体列表: {type}, 数量: {len(agents)}")
        return agents
...
async def export_evaluation_results(
    *,
    ...
    try:
        # 这里应该实现实际的导出逻辑
        # 暂时返回下载链接
        logger.info(f"用户 {current_user.id} 导出评测结果: {len(export_request.session_ids)} 个会话")
