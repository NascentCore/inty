...
def create_agent(
    *,
    ...
    创建新的AI角色
    """
    agent = agent_service.create_agent(db, agent_in=agent_in, user_id=current_user.id)
    return agent
...
def update_agent(
    *,
    ...
    if not agent:
        raise HTTPException(status_code=404, detail="Agent not found")
    agent = agent_service.update_agent(db, db_agent=agent, agent_in=agent_in)
    return agent
...
def delete_agent(
    *,
    ...
    if not agent:
        raise HTTPException(status_code=404, detail="Agent not found")
    agent = agent_service.delete_agent(db, db_agent=agent)
    return agent 