...
# 转换为响应模型
        ...
        response = SystemSettingsListResponse(total=len(items), items=items)
...
async def clear_system_settings_cache(
    ...
    try:
        ...
        message = (
            f"Cache cleared for setting: {key}" if key else "All settings cache cleared"
        )
        return APIResponse.success(data={"message": message}, message=message)
