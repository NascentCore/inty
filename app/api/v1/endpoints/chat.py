...
def _build_chat_response(
    ...
    """构建聊天响应数据"""
    # TODO(companion-ws-wire-types-phase2): WS paths call build_companion_ws_completion_data
    # (ChatWsCompletionData) instead of manual dict assembly; HTTP keeps dict append paths.
...
def _companion_ai_meta_from_turn_result(
    ...
    *,
    ...
    # TODO(companion-ws-wire-types-phase2): return ChatWsCompanionWireMessageMetaData;
    # persist via dump_chat_ws_companion_wire_meta at call sites.
    ...
    try:
        ...
        try:
            ...
            with log_time(f"AI聊天处理: session_id={session_id}"):
                ...
                logger.debug(
                    "chat_turn http user={} chat_id={} agent_id={} model={} subscribed={} chat_llm_api_base={}",
                    current_user.id,
                    chat.id,
                    agent_id,
                    model_override,
                ...
                phone_call_trigger_number = (
                    phone_call_service.extract_call_me_at_number(last_user_text)
                )
