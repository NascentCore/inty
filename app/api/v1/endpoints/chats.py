...
async def agent_chat_completions(
    *,
    ...
    try:
        ...
        logger.info(f"Request ID: {uuid.uuid4()}")
        ...
        logger.info(
            f"Complete Request Data: {json.dumps(request_data, ensure_ascii=False, indent=2)}"
        )
        ...
        if request.messages:
            ...
            for i, msg in enumerate(request.messages):
                ...
                logger.info(
                    f"  Content Length: {len(msg.content) if msg.content else 0}"
                )
                logger.info(
                    f"  Content Preview: {msg.content[:200]}{'...' if len(msg.content) > 200 else ''}"
                )
                if msg.content and len(msg.content) > 200:
                    logger.debug(f"  Full Content: {msg.content}")
        else:
            logger.warning("No messages found in request")

        logger.info(f"=== REQUEST PROCESSING START ===")
        ...
        logger.info(f"=== USER MESSAGE EXTRACTED ===")
        logger.info(f"User Message Length: {len(last_user_message)}")
        logger.info(f"User Message Content: {last_user_message}")
        ...
        if request.stream:
            logger.info(f"=== STREAMING RESPONSE START ===")
            logger.info(f"Streaming enabled, returning StreamingResponse")
            ...
            # 非流式聊天（异步）
            logger.info(f"=== NON-STREAMING RESPONSE START ===")
            ...
            logger.info(f"Response ID: {response_data['id']}")
            logger.info(f"Response Timestamp: {time.strftime('%Y-%m-%d %H:%M:%S')}")
            logger.info(f"Total Processing Time: {total_request_time:.3f}秒")
            logger.info(f"Response Content Length: {len(response_content)}")
            logger.info(f"Response Content: {response_content}")
            logger.info(f"Audio URL: {audio_url if audio_url else 'None'}")
            ...
            response_log_data = {
                ...
                "processing_time_seconds": total_request_time,
                "voice_enabled": chat_settings.voice_enabled if chat_settings else None,
            }
            logger.info(
                f"Complete Response Data: {json.dumps(response_log_data, ensure_ascii=False, indent=2)}"
            )
...
async def agent_chat_fast_response(
    *,
    ...
    try:
        ...
        # ==================== FAST CHAT REQUEST LOGGING ====================
        logger.info(f"=== FAST CHAT REQUEST START ===")
        logger.info(f"Request ID: {uuid.uuid4()}")
        logger.info(f"Agent ID: {agent_id}")
        logger.info(f"User ID: {current_user.id}")
        logger.info(f"User Email: {current_user.email}")
        ...
        logger.info(
            f"Fast Chat Request Data: {json.dumps(request_data, ensure_ascii=False, indent=2)}"
        )

        # Log individual messages for debugging
        if request.messages:
            logger.info(f"=== FAST CHAT MESSAGE DETAILS ===")
            for i, msg in enumerate(request.messages):
                logger.info(f"Message {i+1}:")
                logger.info(f"  Role: {msg.role}")
                logger.info(
            ...
            logger.warning("No messages found in fast chat request")

        logger.info(f"=== FAST CHAT PROCESSING START ===")
        ...
        try:
            ...
            last_user_message = user_messages[-1].content
            logger.info(f"=== FAST CHAT USER MESSAGE EXTRACTED ===")
            logger.info(f"User Message Length: {len(last_user_message)}")
            logger.info(f"User Message Content: {last_user_message}")
        except Exception as e:
        ...
        logger.info(
            f"AI Response Generated: {response_content[:100]}..., 耗时: {ai_time:.3f}秒"
        )
        ...
        settings_time = time.time() - settings_start_time
        logger.info(
            f"Chat Settings Retrieved: voice_enabled={chat_settings.voice_enabled}, 耗时: {settings_time:.3f}秒"
        )
        ...
        if chat_settings.voice_enabled:
            logger.info(f"=== FAST CHAT VOICE PROCESSING START ===")
            ...
            if cached_audio_url:
                ...
                audio_cached = True
                logger.info(f"Fast Chat - Cache Hit: {cached_audio_url}")
            else:
                # 缓存未命中，启动异步生成
                ...
                audio_cached = False
                logger.info(f"Fast Chat - Async Generation Started: {task_id}")

            voice_time = time.time() - voice_start_time
            logger.info(f"Voice Processing Completed: 耗时: {voice_time:.3f}秒")
        ...
        # Add voice-related fields to message if applicable
        if audio_url:
            message["audio_url"] = audio_url
        if audio_task_id:
            message["audio_task_id"] = audio_task_id
        if audio_cached is not None:
            message["audio_cached"] = audio_cached

        total_request_time = time.time() - request_start_time

        # ==================== FAST CHAT RESPONSE LOGGING ====================
        logger.info(f"=== FAST CHAT RESPONSE ===")
        logger.info(f"Response ID: {response_data['id']}")
        logger.info(f"Response Timestamp: {time.strftime('%Y-%m-%d %H:%M:%S')}")
        logger.info(f"Total Processing Time: {total_request_time:.3f}秒")
        logger.info(f"AI Generation Time: {ai_time:.3f}秒")
        logger.info(f"Settings Retrieval Time: {settings_time:.3f}秒")
        logger.info(f"Response Content Length: {len(response_content)}")
        logger.info(f"Response Content: {response_content}")
        logger.info(f"Audio URL: {audio_url if audio_url else 'None'}")
        logger.info(f"Audio Task ID: {audio_task_id if audio_task_id else 'None'}")
        logger.info(f"Audio Cached: {audio_cached}")
        logger.info(f"Voice Enabled: {chat_settings.voice_enabled}")
        logger.info(f"Token Usage: {response_data['usage']}")
        ...
        response_log_data = {
            ...
            "token_usage": response_data["usage"],
            "processing_time_seconds": total_request_time,
            "ai_generation_time_seconds": ai_time,
            "settings_retrieval_time_seconds": settings_time,
            "user_message": last_user_message,
        }
        logger.info(
            f"Fast Chat Complete Response Data: {json.dumps(response_log_data, ensure_ascii=False, indent=2)}"
        )

        logger.info(f"=== FAST CHAT REQUEST END ===")

        return response_data

    except Exception as e:
        logger.error(f"=== FAST CHAT ERROR ===")
        logger.error(f"Error Type: {type(e).__name__}")
        logger.error(f"Error Message: {str(e)}")
        logger.error(f"Agent ID: {agent_id}")
        logger.error(f"User ID: {current_user.id if current_user else 'None'}")
        logger.error(f"Request Data: {request.dict() if request else 'None'}")
        logger.exception("Fast chat error details:")
...
async def generate_chat_stream(
    ...
    try:
        ...
        logger.info(f"=== STREAMING CHAT START ===")
        logger.info(f"Agent ID: {agent_id}")
        ...
        logger.info(f"Streaming Start Time: {time.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        async for message_chunk, metadata in agent.chat_stream(
            ...
            if hasattr(message_chunk, "content") and message_chunk.content:
                chunk_count += 1
                total_content_length += len(message_chunk.content)

                logger.debug(f"Streaming Chunk {chunk_count}:")
                logger.debug(f"  Content: {message_chunk.content}")
                logger.debug(f"  Content Length: {len(message_chunk.content)}")
        ...
        logger.info(f"=== STREAMING CHAT COMPLETE ===")
        logger.info(f"Total Chunks: {chunk_count}")
        logger.info(f"Total Content Length: {total_content_length}")
        logger.info(f"Stream Duration: {stream_duration:.3f}秒")
        logger.info(
            f"Average Chunk Size: {total_content_length / chunk_count if chunk_count > 0 else 0:.1f} characters"
        )
        logger.info(f"Streaming End Time: {time.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        streaming_summary = {
            ...
            "average_chunk_size": (
                total_content_length / chunk_count if chunk_count > 0 else 0
            ),
            "user_message": last_user_message,
        }
        logger.info(
            f"Streaming Summary: {json.dumps(streaming_summary, ensure_ascii=False, indent=2)}"
        )

    except Exception as e:
        logger.error(f"=== STREAMING CHAT ERROR ===")
        logger.error(f"Error Type: {type(e).__name__}")
        logger.error(f"Error Message: {str(e)}")
        logger.error(f"Agent ID: {agent_id}")
        ...
        logger.exception("Streaming chat error details:")
