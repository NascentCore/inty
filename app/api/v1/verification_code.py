...
def send_verification_code(
    *, db: Session = Depends(deps.get_db), verification_code_in: VerificationCodeCreate
) -> dict:
    """
    ...
    verification_code = verification_code_service.create_verification_code(
        db=db, phone=verification_code_in.phone
    )
...
def verify_code(
    *, db: Session = Depends(deps.get_db), verification_code_in: VerificationCodeVerify
) -> dict:
    """
    ...
    verification_code = verification_code_service.get_valid_verification_code(
        db=db, phone=verification_code_in.phone, code=verification_code_in.code
    )
    ...
    verification_code_service.mark_code_as_used(
