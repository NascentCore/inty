...
- `/api/v1/chat/ws`：正式对话 WebSocket，走持久化对话流程（写入 chat 历史）。**产品意图：** `user_signed_on` 控制帧除登记 proactive heartbeat 坐标外，还应驱动「用户上线 → 智能体觉察 → 主动问候」的拟人体验（详见 [`/docs/FR_USER_SIGN_ON_GREETINGS.md`](/docs/FR_USER_SIGN_ON_GREETINGS.md) § Product intent）。`app.features.companion_ws_proactive_heartbeat_enabled`（默认 True）：客户端须发送 `{"type":"user_signed_on","agent_id":...}` 登记坐标后，才在连接存活期间按 `companion_ws_proactive_heartbeat_poll_seconds` 轮询并可能主动下行 companion proactive 轮次；`user_signed_on_ack` 为控制回包。可在 YAML 中设为 false 关闭；关闭时 `user_signed_on` 会 `ok:false`（`proactive_heartbeat_disabled`）。`Pydantic` 模型见 `app.schemas.chat.ChatWsUserSignedOnFrame`。
