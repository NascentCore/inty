from sqlalchemy import Column, String, DateTime, ForeignKey, Boolean, Text, Integer, Float, Enum, JSON
...
class EvaluationSession(Base):
    ...
    creator_id = Column(String, ForeignKey("users.id"), nullable=False, comment="创建者ID")
    status = Column(Enum(EvaluationStatus, name="evaluation_status"), 
                   default=EvaluationStatus.PENDING, comment="评测状态")
    ...
    interactions = relationship("EvaluationInteraction", back_populates="session", cascade="all, delete-orphan")
...
class EvaluationInteraction(Base):
    ...
    chat_id = Column(String, ForeignKey("chats.id"), nullable=True, comment="关联的聊天会话ID")
