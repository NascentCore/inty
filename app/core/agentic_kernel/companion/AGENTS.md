...
- **进程内 registry**：带 repository 的 `MemoryStore` 在 [`memory_registry.py`](/app/core/agentic_kernel/companion/memory_registry.py) 中仅以 **`CompanionScope.registry_key()`**（`user_id:companion_id:chat_id`）注册与复用；`get_memory_store(scope, dsn=...)` 为唯一入口，工具与 `run_turn` 边界通过 **`MemoryStore` 实例**（及线程局部的 runtime inspect overlay）对齐同一 ORM 写入面。
