...
TODO(#3398): ``USER_CHAT`` uses in-turn sync prompts; dual-LLM foreground contract only if
``llm_loop_mode`` restores async split — child #3369.
"""
    ...
    *,
    ...
    match track:
        ...
        case CompanionTurnTrack.USER_CHAT:
            match route_mode:
                case (
                    TurnRouteMode.IN_TURN_SYNC_TOOL
                    | TurnRouteMode.CHAT_ONLY_SYNC
                ):
                ...
                case _:
                    raise RuntimeError(
                        "user_chat track requires IN_TURN_SYNC_TOOL or CHAT_ONLY_SYNC, got "
                        f"{route_mode.value}"
                    )
...
def companion_turn_tools_and_system_messages(
    *,
    ...
    ``USER_CHAT_BOOTSTRAP`` and settled ``USER_CHAT`` run in-turn sync tool loops when
    tools are on (``IN_TURN_SYNC_TOOL``).  Maintenance and autonomy inner ticks use async
    ``tool_background``.  Proactive, scheduled, and implicit sign-on greeting tracks are
    chat-only system stacks with no tools.
    """
...
def refresh_companion_turn_prompt_stack(
    *,
    ...
    route_mode: TurnRouteMode | None = None,
) -> list[dict[str, Any]]:
    """
    ...
    if resolved_route_mode is None:
        resolved_route_mode = resolve_turn_route_mode(
            inner_tick_turn=inner_tick_turn,
            inner_tick_activity=inner_tick_activity,
            tools_enabled=bool(tools_for_turn),
        )
