...
def _output_contract_text_chat_branch_mirrored_tools() -> str:
    if inty_runtime_mode_is_debug():
        runtime_detail_clause = (
            "可核对事实由并行工具路自愿调用 `companion_runtime_inspect` 取得 JSON 后再由该路收口。\n\n"
        )
        tool_branch_tools = "读档案、联网、生图、`companion_runtime_inspect` "
    else:
        runtime_detail_clause = (
            "凡用户追问本助手**如何工作**的内部机制（含模型与调用参数、上下文窗口、注入的 system/对话栈、"
            "工具、记忆管线、后台任务、日志等任何运行时实现细节），"
        ...
        tool_branch_tools = "读档案、联网、生图等须核对或慢步骤。"
    return (
...
def _tool_side_compact_directive() -> str:
    if inty_runtime_mode_is_debug():
        ...
        runtime_status_clause = _prod_runtime_implementation_boundary_clause()
        tool_examples = "（联网检索、生图/改图、档案、持久化约定文档路径工具等）"
    return (
        "## 工具侧（后台 / 系统 2）\n\n"
