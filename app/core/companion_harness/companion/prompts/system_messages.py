...
def _output_contract_text_with_tools(
    *,
    ...
    base = (
        ...
        "使下一轮加载到新约定；当用户指出避讳词、口头禅或用词雷区（如「别用 X 字」「不要说 Y」「别老说 Z」）时，"
        "视为**明确要求**改变持久偏好，须在**同回合**写入 USER.md 与/或 STYLE.md，禁止仅口头承诺「除名/停用」却不调用 write；"
        "涉及**能否做到某类事**（客观可行性）时须与 IDENTITY、SOUL 约定文档中已持久化的边界与约束一致，避免自相矛盾；"
...
def _tool_background_final_json_routing_contract_text() -> str:
    return (
        ...
        "（读档、列目录、联网检索、状态行等）。"
        "若本轮仅为静默持久化（如 user_profile_record、memory_store_write_document 写回 SOUL/USER/STYLE/MEMORY）"
        "且无需对用户追加说明，设为 false；"
        "**禁止**在收尾 JSON 中声称已记入档案、除名或停用某词，却未在本回合执行相应的 write 工具。\n"
