...
_RUNTIME_EVENTS_JSONL_REL = (
    DEFAULT_MEMORY_STORE_SCOPE_PATHS.companion_runtime_events_jsonl
)
...
def append_runtime_event(store: MemoryStore, record: dict[str, Any]) -> None:
    ...
    store.append_jsonl_record(_RUNTIME_EVENTS_JSONL_REL, record)
...
def read_runtime_events(
    ...
    *,
    ...
    raw = store.read_document_if_exists(_RUNTIME_EVENTS_JSONL_REL)
    if not raw:
        return []
