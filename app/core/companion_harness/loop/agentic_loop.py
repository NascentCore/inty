...
from app.core.companion_harness.loop.chat_only_prompt_plan import (
    run_chat_only_prompt_plan,
)
from app.core.companion_harness.loop.prompt_plan_tool_loop import (
    run_prompt_plan_tool_loop,
)
...
class AgenticLoop:
    ...
    async def run_single_llm_turn(
        ...
        if execution.max_tool_call_rounds == 0:
            sync_result = await run_chat_only_prompt_plan(
                context,
                llm_client=self.llm_client,
                appender=appender,
            )
        else:
            sync_result = await run_prompt_plan_tool_loop(
                context,
                store=self.store,
                llm_client=self.llm_client,
                interim_output_sink=(
