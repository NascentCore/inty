...
def _repl_tool_allowed_names() -> frozenset[str]:
    names = {tool.name.value for tool in COMPANION_LLM_TOOLS}
    if not inty_runtime_mode_is_debug():
        names.discard(CompanionToolName.COMPANION_RUNTIME_INSPECT.value)
...
def build_openai_repl_tools(
    *, interactive_bootstrap_active: bool = False
    ...
    if interactive_bootstrap_active:
        names: tuple[CompanionToolName, ...] = REPL_TOOL_NAMES_SHARED_HEAD
    else:
        names = REPL_TOOL_NAMES_SHARED_HEAD + REPL_TOOL_NAMES_NON_BOOTSTRAP_TAIL
    if disable_status:
        names = tuple(
            n for n in names if n != CompanionToolName.TOOL_UPDATE_AGENT_STATUS_LINE
        )
    ...
    appended: tuple[CompanionToolName, ...] = REPL_TOOL_NAMES_APPENDED
    if not inty_runtime_mode_is_debug():
        appended = tuple(
            n for n in appended if n != CompanionToolName.COMPANION_RUNTIME_INSPECT
        )
    out.extend(
        openai_tools_for_names(
            appended,
