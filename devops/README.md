...
- dev 环境 100% 追踪（远程 `inty-backend-dev` 当前常停；`inty-ops-dev` 与本地开发仍可用）
...
- **IntelliMate 数据库（dev + prod）**：均使用 **VM 上 Docker Postgres**（容器 `inty-pg`，逻辑库 `inty-dev` / `inty`），不再连 Cloud SQL；见 [LOCAL_POSTGRES.md](LOCAL_POSTGRES.md)
- **IntelliMate prod compat 部署与 Alembic**：release 客户端兼容后端分支 `intellimate-client-compat-local-postgres-prod`；`inty` 逻辑库 migration 版本须与所部署镜像一致，见 [rollback_records/2026-06-19-inty-pg-alembic-compat-prod.md](rollback_records/2026-06-19-inty-pg-alembic-compat-prod.md)
...
- **VM 部署现状**（容器、端口、日志、已停服务）：[DEPLOYMENT_STATE.md](DEPLOYMENT_STATE.md)
- **iMate（第二 Inty 后端实例）**：与 IntelliMate **并行**，独立库、独立 GCS bucket、独立容器与域名；**不得**在 iMate 流程中 stop/rm `inty-backend-prod`（prod 仍在线）；`inty-backend-dev` 当前**有意停止**，见 [DEPLOYMENT_STATE.md](DEPLOYMENT_STATE.md)。
...
- 操作 dev/prod 环境必须先写 python 脚本，严禁直接操作数据库、或者直接调用管理员权限的 API Endpoint。dev 后端容器当前常停，需要时在 VM 上 `docker start inty-backend-dev` 或见 [DEPLOYMENT_STATE.md](DEPLOYMENT_STATE.md)。步骤如下（dev 后端运行中时）：
  ```bash
  ssh <gcp-vm>
...
- [VM 部署现状（容器 / 端口 / 日志）](DEPLOYMENT_STATE.md)
- [IntelliMate 本地 Postgres（Docker，dev + prod）](LOCAL_POSTGRES.md)
- [Prod rollback / Alembic compat 记录（2026-06-19）](rollback_records/README.md)
- [2026-07-02 VM 部署态变更记录](rollback_records/2026-07-02-disable-gcplogs-and-vm-state.md)
...
- **docker/**：运维侧的 Docker 相关材料（Dockerfile 等）
- **部署现状与日志**：[DEPLOYMENT_STATE.md](DEPLOYMENT_STATE.md)；本机拉取 VM 日志 [fetch_inty_vm_container_logs.sh](fetch_inty_vm_container_logs.sh)
