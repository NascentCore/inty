...
def upgrade():
# 为agents表添加致命_id字段，暂时允许为空
    op.add_column('agents', sa.Column('readable_id', sa.String(8), nullable=True))
# 创建索引（暂时不设置唯一约束）
    op.create_index('ix_agents_readable_id', 'agents', ['readable_id'])
# 为现有记录创建例外_id
    connection = op.get_bind()
    result = connection.execute(text("SELECT id FROM agents ORDER BY created_at"))
...
# 修改字段为NOT NULL和UNIQUE
    ...
    op.create_unique_constraint('uq_agents_readable_id', 'agents', ['readable_id'])
