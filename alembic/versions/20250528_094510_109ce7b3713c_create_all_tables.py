...
# 创建枚举类型
    ...
    authtype = postgresql.ENUM('PHONE', 'GOOGLE', 'GUEST', name='authtype', create_type=False)
    visibility = postgresql.ENUM('PUBLIC', 'PRIVATE', name='visibility', create_type=False)
    agentstatus = postgresql.ENUM('PENDING', 'APPROVED', 'REJECTED', name='agentstatus', create_type=False)
    messagetype = postgresql.ENUM('TEXT', 'VOICE', 'IMAGE', name='messagetype', create_type=False)
...
#创建设置表
    op.create_table('settings',
        sa.Column('id', sa.VARCHAR(), nullable=False),
        sa.Column('user_id', sa.VARCHAR(), nullable=False),
        sa.Column('language', sa.VARCHAR(), server_default=sa.text("'en'::character varying"), nullable=False),
        sa.Column('voice_enabled', sa.BOOLEAN(), server_default=sa.text('true'), nullable=False),
        sa.Column('keep_talking', sa.BOOLEAN(), server_default=sa.text('true'), nullable=False),
        sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.Column('updated_at', postgresql.TIMESTAMP(timezone=True), nullable=True),
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], ),
        sa.PrimaryKeyConstraint('id'),
