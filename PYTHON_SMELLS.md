...
以下为在本仓库中检出的具有典型的Python代码气味（含文件与片段示例）与改进建议。仅列出最重要、最具风险或最广泛分布的模式。

## 1.宽泛/裸异常捕获（Exception/ except:除外）
...
- 建议：用 `loguru.logger`；避免模块进口时打印；敏感信息脱敏。
...
## 7.使用评估
- 代表位置：
  -`experimental/agent_tags_migration/tag_parser.py:160` 执行 `eval(json_str, {"__builtins__": {}}, {})`- 风险：安全风险（取消内置仍可能有逃逸风险）。
-建议：严格改用`json.loads`或专用解析器。

## 8.TODO/FIXME/HACK 长期遗产
...
-建议：定义错误分级与返回规范；为用户状态错误返回致命提示，为系统状态错误保留内部日志与追踪ID。

---

###建议的工程治理措施
- 启用并收紧linter规则（flake8/ruff + mypy + black/ruff 格式）；
- 在CI中新增“禁止裸/宽泛异常”规则白名单机制；
- 引入统一错误模型与异常基类；
- 逐步移除`print`；
