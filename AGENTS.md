...
- IntelliMate 定位为面向 35+、有较好社会地位与自我认知的美国男性用户的长期 AI 陪伴产品，提供可信赖、持续进化的情感陪伴。
- IntelliMate 产品形态基于 Chat 界面，与用户通过手机屏幕交流，使用文字、图片、
声音（语音消息、通话、音乐）、视频（动图、视频、背景声音等）。
- iMate 是所有角色的泛称。
- IntelliMate 是一个以 iMate 为主要产品的体验工厂，无法通过人工设计来满足所有人，只能通过角色让用户自主获得期待的体验
...
- **TDD**：在可行的时候，考虑采用测试驱动开发方式，首先编写测试来预演目标行为，然后通过迭代代码来完成测试
