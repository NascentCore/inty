...
- 只支持 portrait 显示；不支持 landscape 显示，无需在改动时考虑兼容 landscape 显示。
 ...
 - 所有 Compose 组件的 UI 配置必须通过参数传入（可提供默认值），禁止在函数体内硬编码 10.dp/10.sp/10.pt 等魔法值。
 - 在 `android_app/core/design/` 统一定义并导出设计令牌（tokens），如 `Spacing`、`Radii`、`Elevation`、`IconSize`、`AnimationDurations`、`TextStyleTokens`；默认值应引用这些令牌或 `MaterialTheme`。
 - 颜色与排版：优先使用 `MaterialTheme.colorScheme` 与 `MaterialTheme.typography`；必要时通过 `CompositionLocal` 暴露自定义令牌。
 - 容器组件应转发子组件的关键 UI 配置（如内边距/圆角/间距），避免把子项样式写死在容器内部。
 - 可达性：字号用 `sp`，尺寸用 `dp`，遵循系统缩放；如需常量化值请在 tokens 中定义，而非在组件内部直接写数字。
