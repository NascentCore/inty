...
- Structure your answer in the following format:
  - Start with /1sentence summary
  - Followed by /1paragraph key points
  - Do not include additional details unless requested by the user
- When generating a new file from scratch, write a marker to state it was generated by you entirely

## Writing code

Read [agent_conduct_and_code](/.agents/guidelines/agent_conduct_and_code.md) for more guideliens.
...
- Use env vars to control non-functional behaviors, like: logging
...
- Package docstring in `__init__.py`, `__init__.py` should only has docstring.
  Document the package's design and intention.
