...
其中 `test_gcs_service.py` 包含 `upload_live_chat_audio` 的 mock 测试。
...
- **聊天历史**：该会话在聊天历史中有两条消息（一条用户、一条 AI），且均带有 `audio_url`，指向 `live_chat/{user_id}/{agent_id}/{session_id}.wav` 的 GCS URL。
