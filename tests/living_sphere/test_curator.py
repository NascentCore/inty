...
def test_curator_merges_pending_into_living_sphere_md(tmp_path: Path) -> None:
    ...
    assert (
        compact_living_sphere_if_pending(
            store, fake_complete, tool_bg_idle_event=idle
        )
        is True
    )
...
def test_curator_drains_more_than_batch_cap_without_skipping(
    tmp_path: Path,
) -> None:
    ...
    assert (
        state["living_sphere_curated_through_update_id"]
        == updates[-1].update_id
    )
    assert (
        compact_living_sphere_if_pending(
...
def test_curator_rejects_bad_output_without_advancing_cursor(
    tmp_path: Path,
) -> None:
    ...
    assert (
        living_sphere_curator_output_rejection_reason("too short") is not None
    )
...
def test_compact_living_sphere_batch_raises_on_rejected_output(
    tmp_path: Path,
) -> None:
