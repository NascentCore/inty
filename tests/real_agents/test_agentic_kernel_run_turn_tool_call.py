...
def _require_real_agentic_kernel_llm_test() -> None:
    if os.getenv("INTY_AGENTIC_KERNEL_REAL_LLM_TEST") != "1":
        pytest.skip(
            "Set INTY_AGENTIC_KERNEL_REAL_LLM_TEST=1 to run agentic kernel real LLM tests"
        )
    if not (os.getenv("OPENROUTER_API_KEY") or "").strip():
        pytest.skip("OPENROUTER_API_KEY is required for agentic kernel real LLM tests")
