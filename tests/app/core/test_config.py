...
class TestAppConfig:
    def test_app_config_defaults(self):
        ...
        assert config.limits.free_user_total_chat_message_limit == 1000
...
class TestSubscriptionPlan:
    def test_subscription_plan_required_fields(self):
        """Test SubscriptionPlan with required fields"""
        config = SubscriptionPlan(id="test-plan-id", name="Test Plan")
        assert config.id == "test-plan-id"
        assert config.name == "Test Plan"

    def test_subscription_plan_defaults(self):
        """Test SubscriptionPlan default values"""
        config = SubscriptionPlan(id="test-plan-id", name="Test Plan")
        assert config.description is None
        assert config.plan_type == "MONTHLY"
        assert config.price == 0.0
        assert config.currency == "USD"
        assert config.google_play_product_id == ""
        assert config.discount_rate == 1.0
        assert config.features == {}
        assert config.chat_limit_per_day == -1
        assert config.agent_creation_limit == 6
        assert config.background_generation_limit_per_day == 3
        assert config.is_active is True
        assert config.sort_order == 0

    def test_subscription_plan_custom_values(self):
        """Test SubscriptionPlan with custom values"""
        config = SubscriptionPlan(
            ...
            price=99.99,
            currency="EUR",
            google_play_product_id="premium_yearly",
            discount_rate=0.8,
            features={"voice": True, "background": True},
            chat_limit_per_day=1000,
            agent_creation_limit=20,
            background_generation_limit_per_day=10,
            is_active=True,
            sort_order=1,
        )
        assert config.id == "premium-plan"
        assert config.name == "Premium Plan"
        assert config.description == "Premium subscription with full features"
        assert config.plan_type == "YEARLY"
        assert config.price == 99.99
        assert config.currency == "EUR"
        assert config.google_play_product_id == "premium_yearly"
        assert config.discount_rate == 0.8
        assert config.features == {"voice": True, "background": True}
        assert config.chat_limit_per_day == 1000
        assert config.agent_creation_limit == 20
        assert config.background_generation_limit_per_day == 10
        assert config.is_active is True
...
class TestConfig:
    def test_config_creation(self):
        ...
        subscription_plan = SubscriptionPlan(id="test-plan", name="Test Plan")
        ...
        assert config.subscription_plans == [subscription_plan]
...
class TestLoadConfig:
    def test_load_config_success(self):
        ...
        try:
            ...
            assert len(config.subscription_plans) == 2
            assert config.subscription_plans[0].id == "basic-plan"
            assert config.subscription_plans[0].name == "Basic Plan"
            assert config.subscription_plans[0].plan_type == "MONTHLY"
            assert config.subscription_plans[0].price == 9.99
            assert config.subscription_plans[1].id == "premium-plan"
            assert config.subscription_plans[1].name == "Premium Plan"
            assert config.subscription_plans[1].plan_type == "YEARLY"
            assert config.subscription_plans[1].price == 99.99
        finally:
