"""Inner-tick async tool_background path (kernel)."""
...
class _FakeInnerTickLLMClient:
    ...
    def chat_completion(self, **kwargs: Any) -> Any:
        ...
        env = {
            "user_facing_reply": "proactive ok",
            "importance_round": 5,
...
async def test_async_dual_inner_tick_passes_tick_context_and_inner_tick_tools(
    ...
    client = _FakeInnerTickLLMClient()
...
async def test_proactive_inner_tick_proactive_chat_sync_still_calls_llm(
    ...
    """PROACTIVE inner tick is PROACTIVE_CHAT_SYNC (no async tool_background branch)."""
    store = _store(tmp_path)
    store.write_document("context.json", '{"context_mode": "intimate"}\n')
    store.write_document("IDENTITY.md", "id\n")
    store.write_document("SOUL.md", "s\n")
