...
def test_scope_listing_transcript_kind_matches_accessor() -> None:
    rel = DEFAULT_MEMORY_STORE_SCOPE_PATHS.transcript
    kind, calendar_date = parse_memory_store_relative_path(rel)
    assert kind == CompanionMemoryDocumentKind.TRANSCRIPT
    assert calendar_date is None


def test_scope_listing_transcript_inner_tick_kind_matches_accessor() -> None:
    rel = DEFAULT_MEMORY_STORE_SCOPE_PATHS.transcript_inner_tick
    kind, calendar_date = parse_memory_store_relative_path(rel)
    assert kind == CompanionMemoryDocumentKind.TRANSCRIPT_INNER_TICK
    assert calendar_date is None


def test_scope_listing_dreaming_state_json_kind_matches_accessor() -> None:
