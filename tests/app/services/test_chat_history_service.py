...
class TestChatHistoryService:
    ...
    def test_parse_message_content_system_type(self) -> None:
        ...
        assert _parse_message_content(raw) == {
            "content": "gate",
            "role": "system",
        }

    @pytest.mark.asyncio
    async def test_get_latest_ai_message_info_returns_none_when_no_chat_history(
        self,
    ):
        ...
        result = await get_latest_ai_message_info(
            mock_db, "non-existent-session-id"
        )
    ...
    @pytest.mark.asyncio
    async def test_get_latest_user_message_id_returns_none_when_no_user_message(
        self,
    ):
        ...
        mock_db.execute = AsyncMock(return_value=mock_result)
        result = await get_latest_user_message_id(
            mock_db, "no-user-msg-session"
        )
        assert result is None

    @pytest.mark.asyncio
    async def test_get_latest_user_message_id_returns_id_when_user_message_exists(
        self,
    ):
