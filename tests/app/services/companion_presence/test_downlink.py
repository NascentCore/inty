"""CompanionDownlink factories and delivery gating."""
...
def test_user_reply_downlink_binds_turn() -> None:
    ...
    assert event.kind is CompanionDownlinkKind.USER_REPLY
...
def test_proactive_downlink_carries_transcript_user_text() -> None:
    ...
    assert event.kind is CompanionDownlinkKind.PROACTIVE
...
def test_scheduled_downlink_requires_task_id() -> None:
    ...
    assert event.kind is CompanionDownlinkKind.SCHEDULED
...
def test_bootstrap_interim_downlink() -> None:
    ...
    assert event.kind is CompanionDownlinkKind.BOOTSTRAP_INTERIM
