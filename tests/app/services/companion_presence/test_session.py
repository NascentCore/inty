"""CompanionPresenceCoordinator and CompanionPresenceSession."""
...
class _RecordingDownlink:
    def __init__(self) -> None:
        self.events: list[CompanionDownlink] = []
...
async def test_presence_coordinator_bootstrap_interim_sink_queues_output() -> None:
    coordinator = CompanionPresenceCoordinator.for_current_loop()
...
async def test_presence_session_deliver_bootstrap_interim() -> None:
    ...
    session = CompanionPresenceSession.create(downlink=downlink, loop=loop)
    ...
    assert downlink.events[0].kind is CompanionDownlinkKind.BOOTSTRAP_INTERIM
