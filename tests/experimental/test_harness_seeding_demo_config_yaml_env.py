...
def test_apply_sets_openai_from_agent_api_key(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    cfg = tmp_path / "c.yaml"
    cfg.write_text(yaml.dump({"agent": {"api_key": "sk-test-from-yaml"}}), encoding="utf-8")
...
def test_apply_skips_when_env_already_set(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    cfg = tmp_path / "c.yaml"
...
def test_apply_openai_api_key_top_level(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    cfg = tmp_path / "c.yaml"
