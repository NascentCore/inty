...
| CFG-PYD-21 | claimed in `cursor/agent-maintenance-tasks-2941` | `AgentConfig` | — | `agent` | 体量大；无其它 *Config* dataclass 交叉引用 |
...
| CFG-PYD-23 | claimed in `cursor/agent-maintenance-tasks-2941` | `AppConfig` | CFG-PYD-06, CFG-PYD-20, CFG-PYD-22 | `app` | 含 `backend_cors_origins` 等；修正 `List[AnyHttpUrl] = None` 为 `Optional[...]`；`LimitsConfig` / `features` / `api_endpoints` 默认值用 `Field(default_factory=...)`；`__post_init__` 合并进 validator |
