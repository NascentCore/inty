...
| CFG-PYD-21 | done | `AgentConfig` | — | `agent` | 已迁移为 `BaseModel`；`load_config` 使用 `model_validate`；Fixed in `cursor/agent-maintenance-tasks-2941` |
...
| CFG-PYD-23 | done | `AppConfig` | CFG-PYD-06, CFG-PYD-20, CFG-PYD-22 | `app` | 已迁移为 `BaseModel`；`backend_cors_origins` 修正为 `Optional[...]`；嵌套默认值由 `Field(default_factory=...)` 与 validator 保持；Fixed in `cursor/agent-maintenance-tasks-2941` |
