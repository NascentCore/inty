...
| CFG-PYD-12 | done | `ElevenLabsConfig` | — | `elevenlabs` | 已迁移为 `BaseModel`；必填 `api_key` 用 `Field(...)` 保留；Fixed in `cursor/agent-maintenance-tasks-3e21` |
| CFG-PYD-13 | done | `FalConfig` | — | `fal` | 已迁移为 `BaseModel`；Fixed in `cursor/agent-maintenance-tasks-3e21` |
...
| CFG-PYD-15 | done | `UserAnalyticsReportConfig` | — | `user_analytics_report` | 已迁移为 `BaseModel`；Fixed in `cursor/agent-maintenance-tasks-3e21` |
...
| CFG-PYD-21 | in_progress | `AgentConfig` | — | `agent` | 体量大；无其它 *Config* dataclass 交叉引用；claimed `cursor/agent-maintenance-tasks-02d9` |
...
| CFG-PYD-23 | in_progress | `AppConfig` | CFG-PYD-06, CFG-PYD-20, CFG-PYD-22 | `app` | 含 `backend_cors_origins` 等；修正 `List[AnyHttpUrl] = None` 为 `Optional[...]`；`LimitsConfig` / `features` / `api_endpoints` 默认值用 `Field(default_factory=...)`；`__post_init__` 合并进 validator；claimed `cursor/agent-maintenance-tasks-02d9` |
