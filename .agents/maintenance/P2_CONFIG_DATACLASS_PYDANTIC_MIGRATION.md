...
| task_id | status | 描述 |
|--------|--------|------|
| CFG-PYD-OPT-01 | done | 将 `_validate_config` 中仅依赖单个子树的规则拆入对应 `BaseModel` 的 `model_validator`（required keys、agent nicknames、environment×limits 仍留根校验）；Fixed in `cursor/agent-maintenance-tasks-c162` |
