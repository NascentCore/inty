...
## 2026-09-17 scan (cron)

Source: open PR overlap check (maintenance PRs #3930–#3947 claim HYGIENE-2026-359..401); ruff F401 on `agentic_companion` ws modules; stale issue audit snapshot; `models.py` `OUTPUT_FORMAT_IM_DM_MD` re-export still consumed by ops telegram prompt-stack test.

Open PRs checked: `cursor/agent-maintenance-tasks-*` through pull/3947 — no overlap with tasks below.
