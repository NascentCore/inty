...
## 2026-09-07 scan (cron)

Source: open PR overlap check (#3869..#3921 claim HYGIENE-2026-182..342 — no overlap with tasks below); ruff UP017/UP035/UP041/F401/F841 + vulture `--min-confidence 80` clean; `google_web_search` import broken on `main` (`app.utils.simple_http`); production modules still import `*_REL` from `memory_store_path_constants`.

Open PRs checked: #3869..#3921 (`cursor/agent-maintenance-tasks-*` scope-path / google_web_search batches) — no overlap with tasks below.
