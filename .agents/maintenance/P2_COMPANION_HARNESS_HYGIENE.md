...
## 2026-09-20 scan (cron)

Source: open PR overlap check (#3955 HYGIENE-408..411 image_gate + turn_invariants parity, #3952 HYGIENE-405..407 lifecycle parity, #3949 HYGIENE-402..404 F401/models alias — no overlap); ruff UP017/UP035/UP041/F401/F841 + vulture `--min-confidence 80` clean; follow-up — production MemDoc I/O still imports `*_MD_REL` from `memory_store_path_constants` where `DEFAULT_MEMORY_STORE_SCOPE_PATHS` accessors exist (`client_time_from_memory_store`, `retrieval`, `read_web_page`, `companion_tool_runtime`).

Open PRs checked: #3949, #3952, #3955, scope-listing + stale-harness maintenance PRs — no overlap with tasks below.
