...
## 2026-07-19 scan (stale code review)

Source: stale/legacy code review guided by `docs/imate/companion_harness/DESIGN.md` + `ARCH.md`; ruff F401/F841 + vulture `--min-confidence 80` clean on `app/core/companion_harness/` except the deferred #3834-overlapping F401 in `system_messages.py`. Two superseded legacy helpers found, each kept alive only by its own test (production callers already migrated to successor mechanisms).

Note: `prompting/projection/` and `memory/retrieval.py` Phase-0 stubs are **target-architecture scaffolding** per DESIGN.md § 记忆模型 / ARCH.md, not legacy — left in place.

Open PRs checked: #3834 (`cursor/phase-2-tracksystemrecipe-b95a` — HYGIENE-2026-92/122..124 stay deferred), #3837 (`cursor/long-term-user-simulator-4deb`) — no overlap with tasks below.

### Open tasks

- [x] **HYGIENE-2026-131** #3832 follow-up: remove stale `turn_compose_context_from_legacy_flags` in `prompting/compose_context.py` — an explicit bridge to the `build_system_messages` combiner deleted in #3832; no production caller, only its own test (`test_legacy_flags_use_track_pinned_bootstrap_phase`, duplicating `test_resolve_phase_for_compose_user_chat_bootstrap_pins_bootstrap`).
- [x] **HYGIENE-2026-132** #3459/#3580 follow-up: remove superseded `inner_tick_activity_suppresses_user_delivery` in `companion/models.py` — replaced by `InnerTickKindSpec.suppresses_user_delivery` (`inner_tick_kind.py`) surfaced via `loop/track_policy.py` and consumed in `loop/agentic_loop.py`; only its own test (`test_autonomy_suppresses_user_delivery`) referenced it (successor coverage in `test_inner_tick_kind.py` / `test_track_policy.py`).
