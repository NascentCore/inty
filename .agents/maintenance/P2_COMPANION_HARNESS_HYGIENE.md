...
- [x] **HYGIENE-2026-182** #3453: remove stale `turn_compose_context_from_legacy_flags` (bridge from `build_system_messages` bool flags to `TurnComposeContext`, part of the #3453 prompt-assembly migration); production compose paths use `build_turn_compose_context` / `turn_compose_context_for_self_contained_track` directly, so the bridge had zero production callers and was referenced only by its own test. Deleted the function in `prompting/compose_context.py` plus the orphaned `test_compose_context.py::test_legacy_flags_use_track_pinned_bootstrap_phase` and its now-unused import; the track-pinned bootstrap-phase behavior stays covered by `test_resolve_phase_for_compose_user_chat_bootstrap_pins_bootstrap`. (First commit message mislabeled the tracking issue as #3463; correct issue is #3453.)
