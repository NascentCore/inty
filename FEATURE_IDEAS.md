...
### 一句话总结
基于用户当前情绪/场景/纪念日，自动生成一首“当下的歌”（歌词 + 旋律/编曲 + 人声/合唱），并可在聊天/通话中作为氛围插曲或“共同回忆”收藏。

### 关键用户旅程（简要）
- 在聊天/通话页点击「生成我们的歌」或在纪念日/节日弹出入口（生日、圣诞、跨年、第一次聊天周年等）
- 选择风格与长度：民谣/流行/电子/lofi；30 秒/1 分钟/完整版；独唱/对唱/合唱
- 系统采集最小必要信息：场景关键词、情绪强度、两人称呼与边界（避免过度露骨/敏感内容）
- 生成试听：先出 15 秒 Hook（副歌）→ 用户可微调（更甜/更克制/更俏皮/更燃）→ 再补全
- 发布与沉淀：保存到「回忆专辑」，生成分享卡（封面图 + 歌词摘句），可作为下次聊天的开场 BGM

### 体验与产品要点
- 情绪贴合：歌词引用对话里的“共同梗/关键词”（可开关），让歌曲更像“你们的共同作品”
- 可控与安全：默认 PG-13；敏感词/人设红线过滤；支持“只写不唱”（只生成歌词与和弦进行）
- 低延迟分段生成：先 Hook 再完整版；弱网/高峰时可退化为“歌词 + 伴奏”或“歌词卡片”
- 版权与归属：明确“生成内容的可用范围/商用限制”；提供下载（音频/歌词/工程分轨可选）与水印选项

### 使用到的能力
- LLMs：从对话中提炼主题与意象、生成歌词、生成结构（Verse/Chorus/Bridge）与可唱性约束
- Music Generation：旋律/和声/编曲（可接第三方音乐生成模型或自建管线）
- TTS / Singing Voice：人声演唱（可选多音色、对唱），并支持自动对齐歌词时间轴（LRC）
- Text-to-Image：生成单曲封面（基于场景/情绪）

### Web / Mobile 实现草图
- 前端（Web/移动端）：
  - 生成进度 UI：Hook 先播 + 可视化波形/时间轴；歌词逐行高亮（LRC）
  - 编辑器：风格/速度/调性（简化为 3-5 个档位）+ 文案级“气质滑杆”
  - 导出：MP3/M4A + 封面 + 歌词；分享卡片一键分享到社媒
- 后端：
  - 编排器：对话摘要 → 主题词/押韵方案 → Hook 生成 → 质量检测（可唱性/敏感词/重复度）→ 补全
  - 队列：异步生成音频与封面，Hook 结果优先队列；失败降级到“歌词卡”
  - 存储：作品版本管理（v1/v2），支持“重写副歌/保留旋律”式局部重生成

### 指标与风险
- 指标：入口点击率、Hook 完播率、二次微调率、收藏率、分享率、下次会话回流率
- 风险：生成质量不稳/审美分歧 → 先 Hook 快速试错 + 提供风格模板；版权争议 → 明确授权条款 + 相似度/抄袭检测（可选）
