...
import com.inty.utils.log.xlog.EncryptXlog
import java.io.PrintWriter
import java.io.StringWriter


// output to mars-xlog
class FileInterceptor : Interceptor<Any>() {
    override fun log(tag: String, message: Any, priority: Int, chain: Chain, vararg args: Any) {
        if (isLoggable(message) && EasyLog.curPriorityFile <= priority && EasyLog.curPriorityFile != EasyLog.NONE) {
//            Log.println(priority, tag, getFormatLog(message, *args))
//            com.tencent.mars.xlog.Log.i(tag, getFormatLog(message, *args))
            EncryptXlog.log(priority, tag, getFormatLog(message, *args))
        }
