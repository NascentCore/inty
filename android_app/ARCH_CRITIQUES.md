...
### 范围
- **描述目标**：概述主要功能，当前架构，评估架构与功能的装备度，并首发主要架构问题及其影响。不包含修复方案。

### 功能概述
- **AI聊天**：与智能体对话、历史消息、Keep Talking、TTS语音播放。
- **智能体**：发现、关注、创建/编辑、报告。
- **鉴权**：游客创建、Google登录。
- **订阅**：Google Play 结算套餐与状态监控。
- **个人与设置**：用户资料、应用更新检查、偏好设置。
- **媒体**：图片加载（Coil）、音频播放（Media3/ExoPlayer）。

### 架构摘要
- **模块**：
  -`app`：Compose UI、多个Activity、启动编排。
  -`core/data`：网络层（两套并存）、SDK包装、仓库、聊天领域（仓库/实例）、本地设置（MMKV）。
  -`core/common`、`core/design`、`core/firebase`、`library/network`、`library/utils`：通用UI、工具、分析与网络辅助。
- **UI层**：Compose + MVVM（`BaseVM`+ StateFlow）。多Activity导航+自定义底部栏；导航Compose使用有限。
- **数据层**：
  - Retrofit/Moshi 栈：`NetServiceMgr` + `I*Api` 接口。
  - 生成 SDK 栈：`IntyNetworkManager` + `*Service` 门面，返回 `ApiResult`。
  - 本地存储使用MMKV；无房间；聊天持久化目前禁止。
...
### 改装性评估
- Compose + MVVM的精准整体匹配“聊天优先、富媒体”的产品形态。
- 但风险的两套网络栈、非标准化DI、重度单例编排、以及关闭的持久化，具有可靠性、一致性与可测性；对强调长期的陪伴与稳定体验的产品构成。
...
- **问题**：`ChatModule` 以全局对象装配仓库与用例；存在 `@Inject`注解但无实际 DI 运行时（Hilt/Koin）接管；ViewModel 多为停止拉取依赖。
- **影响**：测试替身注入困难、生命周期与作用域模糊、依赖关系隐藏在单例内部、范围不一致。

### 3）`BaseVM` 中存在脱离生命周期的后台任务
- **问题**：自建 `backgroundScope` 与“持久化”协程，不受 `viewModelScope` 管控。
- **影响**：任务可能在界面销毁后继续执行，易引发泄漏、竞态与无效状态更新。

### 4）`AuthInterceptor`第401章 处理脆弱
- **问题**：遇到401即登出并重启应用程序。
- **影响**：可能出现重启循环；对令牌临时或临时网络问题缺乏温和恢复；与 SDK 鉴权路径间歇。

### 5）初始化依赖固定延迟并与 Activity 耦合
- **问题**：`MainActivity` 以 `delay(500)`等“经验值”启动与拉取套餐；监控从 Activity 启动。
- **影响**：启动流程脆弱、不同设备易竞态、器件生命周期与单一接口耦合。

### 6）聊天持久化被取消，缺乏离线连续性
- **问题**：聊天消息本地持久化方法为无操作，仅存分页标记。
- **影响**：冷启动缺上下文；离线与弱网体验差；网络负载增加。

### 7）导航混合：多Activity + Compose，导航Compose 使用有限
- **问题**：大量通过`Intent`启动新Activity，自定义底部导航；`NavHost` 使用较少。
- **影响**：返回栈复杂；深链/状态恢复困难；过多手工过渡与状态维护；导航用例测试性差。

### 8）OkHttp 客户端重复且配置不一致
- **问题**：`NetServiceMgr`、Media3 、Coil 图片分别创建 OkHttpClient，超时/拦截器中断。
- **影响**：header/TLS/重试/日志/存储策略不统一；难以施加跨切关注点。

### 9）重试拦截器使用暂停休眠（且当前被注释）
...
- **影响**：不同设备/系统版本行为不一致；可能与系统请求冲突；成本高。

## 积极面
- **Compose + StateFlow**：在 ViewModel 中一致使用现代响应式模式。
- **等级**：设计、工具、Firebase、数据与应用层分割语音。
- **媒体能力**：基于Media3的音频播放与焦点管理，图像/音频预加载策略积极。
- **控制器仓库**：统一订阅状态与事件中心（虽存在生命周期关联问题）。