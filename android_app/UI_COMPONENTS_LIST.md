...
本文档详细列出了 Android app 中用户可见的各类 UI 组件的名称及其内涵。每个组件都包含以下信息：
- **位置**: 组件所在的文件路径
- **功能**: 组件的主要功能说明
- **入口**: 组件如何被调用，从哪个父组件或操作进入（新增）
- **组成**: 组件包含的子组件（如适用）
- **交互**: 组件的交互方式（如适用）
- **样式**: 组件的样式特征（如适用）
...
- **入口**: 从 `HomeScreen` 的 Scaffold bottomBar 调用
- **Tab 项**:
  - 首页（Home/Chat）
  - 消息（Messages）
  - 创建（Create）
...
- **位置**: `HomeScreen.kt`
- **功能**: 单个 Tab 项的 UI 组件
- **入口**: 从 `AppBottomNavigationBar` 循环调用，为每个 Tab 创建
- **显示**: 图标 + 文字标签
...
- **功能**: 显示与 AI 角色的对话界面
- **入口**: 
  - 从 `ChatActivity` 调用（独立聊天页面）
  - 从 `ChatPageContainer` 调用（主页面的聊天 Tab）
- **组成**:
  - ChatTopBar（顶部栏）
...
- **功能**: 标识当前角色使用高级模型
- **入口**: 从 `ChatPage` 中调用，位于 ChatTopBar 下方
...
- **位置**: `ChatItem.kt`
- **功能**: 显示单条聊天消息
- **入口**: 从 `ChatPage` 的消息列表（LazyColumn）中调用
- **类型**:
  - ChatItemAI（AI 消息）
  - ChatItemUser（用户消息）
- **组成**:
...
- **位置**: `ChatItem.kt`
- **功能**: 显示 AI 角色的回复消息
- **入口**: 从 `ChatItem` 调用，当消息角色为 "assistant" 时显示
- **特征**:
  - 左侧对齐
  - 深色半透明背景
  - 支持语音播放按钮
...
- **位置**: `ChatItem.kt`
- **功能**: 显示用户发送的消息
- **入口**: 从 `ChatItem` 调用，当消息角色为 "user" 时显示
- **特征**:
  - 右侧对齐
  - 白色半透明背景
  - 支持长按复制
...
- **位置**: `ChatItem.kt`
- **功能**: 显示消息生成中的加载状态
- **入口**: 从 `ChatItemAI` 调用，当消息内容为 "loading_animation" 时显示
- **样式**: 三个跳动的圆点动画

### 11. AgentInfoChatCard（角色介绍卡片）
- **位置**: `ChatItem.kt`
- **功能**: 在聊天页面顶部显示角色介绍信息
- **入口**: 从 `ChatPage` 调用，在消息列表顶部显示（仅当无更多消息或消息列表为空时）
- **特征**:
  - 可展开/折叠文本
  - 显示"Intro:"标签
  - 支持多行文本显示
...
- **位置**: `ChatItem.kt`
- **功能**: 支持展开/折叠的长文本组件
- **入口**: 从 `AgentInfoChatCard` 调用，用于显示可展开的角色介绍文本
- **交互**: 点击箭头按钮展开或折叠
...
- **位置**: `ChatInput.kt`
- **功能**: 快速插入括号用于旁白输入
- **入口**: 从 `ChatInput` 调用，仅在输入框获得焦点时显示
- **交互**: 点击插入"()"括号

### 15. MultiUseAccessButton（多功能按钮）
- **位置**: `ChatInput.kt`
- **功能**: 根据输入状态切换发送/更多按钮
- **入口**: 从 `ChatInput` 调用，位于输入框右侧
- **状态**:
  - 有输入内容：显示发送按钮
  - 无输入内容：显示更多/收起按钮
...
- **位置**: `ChatMorePanel.kt`
- **功能**: 从底部弹出的更多功能面板
- **入口**: 从 `ChatPage` 调用，点击输入框右侧的"更多"按钮（无输入内容时）显示
- **功能项**:
...
- **位置**: `ChatMorePanel.kt`
- **功能**: 更多面板中的单个功能项
- **入口**: 从 `ChatMorePanel` 调用，为每个功能项创建
- **组成**: 图标 + 文字 + VIP 标识（可选）

### 19. ChatSettingsDrawer（聊天设置抽屉）
- **位置**: `ChatSettingsDrawer.kt`
- **功能**: 从右侧滑出的设置抽屉
- **入口**: 从 `ChatPage` 调用，点击 ChatTopBar 右侧的"更多"按钮打开
- **内容**:
  - 我的角色（My Persona）:
    - 名称（Name）
    - 代词（Pronouns）
...
- **入口**: 从 `HomeScreen` 调用，用于主页面的"聊天"Tab，支持左右滑动切换不同角色的聊天页面
...
- **位置**: `ExplorePage.kt`
- **功能**: 显示推荐角色列表的页面
- **入口**: 从 `HomeScreen` 调用，当用户点击底部导航栏的"探索"Tab 时显示
- **组成**:
  - TopAppBar（顶部栏）
...
- **功能**: 自适应布局的标签组件
- **入口**: 从 `ExploreCharacterCard`、`AgentInfoScreen` 等组件调用，用于显示角色标签
- **特征**: 
  - 自动换行，智能计算可显示的标签
  - 支持最大行数限制
...
- **位置**: `MessagesPage.kt`
- **功能**: 显示会话列表的页面
- **入口**: 从 `HomeScreen` 调用，当用户点击底部导航栏的"消息"Tab 时显示
- **组成**:
  - TopAppBar（顶部栏）
  - 会话列表（LazyColumn）
  - 下拉刷新指示器
...
- **位置**: `MessagesPage.kt`
- **功能**: 显示单个会话的列表项
- **入口**: 从 `MessagesPage` 的 LazyColumn 中调用，为每个会话创建
- **组成**:
  - 角色头像（圆形）
  - 角色名称
  - 最后一条消息预览
...
- **功能**: 显示列表为空时的占位组件
- **入口**: 从 `MessagesPage` 等列表页面调用，当列表为空且不在加载状态时显示
- **组成**: 图标 + 提示文字
...
- **位置**: `ProfilePage.kt`
- **功能**: 显示用户个人资料和创建的角色列表
- **入口**: 从 `HomeScreen` 调用，当用户点击底部导航栏的"我的"Tab 时显示
- **组成**:
  - 用户头像和昵称
  - 用户 ID
  - 用户描述
...
- **位置**: `ProfilePage.kt`
- **功能**: 显示用户创建的角色卡片
- **入口**: 从 `ProfilePage` 的 LazyVerticalGrid 中调用，为每个用户创建的角色创建
- **组成**:
...
- **功能**: 图片加载时的占位动画效果
- **入口**: 从 `ExploreCharacterCard`、`MyAgentCard` 等调用，图片加载前显示
- **样式**: 闪烁动画效果
...
- **位置**: `LoginScreen.kt`
- **功能**: 用户登录界面
- **入口**: 从 `LoginActivity` 调用，或从多处需要登录的地方跳转（如点击 VIP 功能、点击角色卡片等）
- **组成**:
...
- **位置**: `LoginUI.kt`
- **功能**: 关闭登录页面的按钮
- **入口**: 从 `LoginScreen` 调用，位于登录页面右上角

### 34. LogoImage（Logo 图片）
- **位置**: `LoginUI.kt`
- **功能**: 显示应用 Logo
- **入口**: 从 `LoginScreen` 调用，位于登录页面中央

### 35. WelcomeTitle（欢迎标题）
- **位置**: `LoginUI.kt`
- **功能**: 显示欢迎文字标题
- **入口**: 从 `LoginScreen` 调用，位于 Logo 下方

### 36. WelcomeSubtitle（欢迎副标题）
- **位置**: `LoginUI.kt`
- **功能**: 显示欢迎文字副标题
- **入口**: 从 `LoginScreen` 调用，位于 WelcomeTitle 下方

### 37. GoogleLoginButton（Google 登录按钮）
- **位置**: `LoginUI.kt`
- **功能**: Google 账号登录按钮
- **入口**: 从 `LoginScreen` 调用，位于欢迎文字下方
- **状态**: 支持加载状态显示

### 38. PolicyText（隐私政策文本）
- **位置**: `PolicyRowUI.kt`
- **功能**: 显示隐私政策和使用条款链接
- **入口**: 从 `LoginScreen` 调用，位于登录按钮下方

### 39. RegInfoScreen（注册信息屏幕）
- **位置**: `RegInfoScreen.kt`
- **功能**: 收集用户注册信息（年龄等）
- **入口**: 从 `RegInfoActivity` 调用，登录后如果用户信息不完整时跳转
...
- **功能**: 显示角色的详细信息页面
- **入口**: 从 `AgentInfoActivity` 调用，点击 ChatTopBar 的角色卡片或 ExploreCharacterCard 跳转
- **组成**:
...
- **功能**: 显示角色的背景图片
- **入口**: 从 `ChatPage`、`AgentInfoScreen` 等调用，作为页面背景
- **特征**: 支持渐变遮罩（可选）

### 42. AgentSpacerLine（角色分隔线）
- **位置**: `AgentInfoScreen.kt`
- **功能**: 信息区域之间的分隔线
- **入口**: 从 `AgentInfoScreen` 调用，用于分隔不同的信息区域
- **样式**: 渐变水平线

### 43. BottomSheetContent（底部菜单内容）
- **位置**: `AgentInfoScreen.kt`
- **功能**: 角色详情页的底部菜单
- **入口**: 从 `AgentInfoScreen` 调用，点击右上角"更多"按钮时显示
- **选项**: 举报、取消
...
- **位置**: `DialogUI.kt`
- **功能**: 提示用户必须更新应用
- **入口**: 从 `HomeScreen` 调用，当检测到需要强制更新时自动弹出
- **特征**: 无法关闭，必须点击更新按钮

### 45. DeleteAccountDialog（删除账号对话框）
- **位置**: `DialogUI.kt`
- **功能**: 确认删除账号的对话框
- **入口**: 从 `SettingContent` 调用，点击"删除账号"选项时显示
- **交互**: 确认删除或取消

### 46. UnlimitChatDialog（无限聊天对话框）
- **位置**: `UnlimitChatDialog.kt`
- **功能**: 提示用户聊天次数限制的对话框
- **入口**: 从 `ChatPage` 调用，当用户达到免费聊天次数限制时自动弹出
- **交互**: 跳转到 VIP 中心或登录页

### 47. ExpiredVipDialog（VIP 过期对话框）
- **位置**: `ExpiredVipDialog.kt`
- **功能**: 提示 VIP 订阅已过期的对话框
- **入口**: 从 `HomeScreen` 调用，当检测到 VIP 订阅过期且首次提示时显示
- **交互**: 跳转到订阅或登录页

### 48. EditDialog（编辑对话框）
- **位置**: `EditDialog.kt`
- **功能**: 编辑用户信息的对话框
- **入口**: 从 `ChatSettingsDrawer`、`MySettingActivity` 等调用，点击可编辑项时显示
- **支持字段**: 名称、代词、人设等

### 49. ReplyStyleSheet（回复风格底部表单）
- **位置**: `ReplyStyleSheet.kt`
- **功能**: 编辑 AI 回复风格的底部表单
- **入口**: 从 `ChatMorePanel` 调用，点击"回复风格"选项时显示（需 VIP）
- **权限**: VIP 专享功能
...
- **位置**: `MySettingUI.kt`
- **功能**: 设置列表中的单个设置项
- **入口**: 从 `ChatSettingsDrawer`、`MySettingActivity` 等调用，用于显示用户信息项
- **组成**: 键值对显示，右侧箭头
...
- **位置**: `TextField.kt`
- **功能**: 自定义的文本输入框
- **入口**: 从 `ChatInput` 调用，作为聊天输入框的主要输入组件
- **特征**: 支持多行、字符限制、焦点管理
...
- **功能**: 设置页面的主内容容器
- **入口**: 
  - 从 `MainActivity` 调用，当 `showSettings` 状态为 true 时显示
  - 从 `SettingActivity` 调用（独立设置页面）
- **组成**:
  - SettingTopBar（顶部栏）
...
- **位置**: `SettingContent.kt`
- **功能**: 设置页面的顶部标题栏
- **入口**: 从 `SettingContent` 的 Scaffold topBar 调用
- **组成**: 标题文字 + 返回按钮

### 71. SettingSection（设置项容器）
- **位置**: `SettingUI.kt`
- **功能**: 设置项的分组容器
- **入口**: 从 `SettingContent` 调用，用于分组多个设置项
- **样式**: 带边框和背景的圆角容器

### 72. SettingSwitchItem（设置开关项）
- **位置**: `SettingUI.kt`
- **功能**: 带开关的设置项
- **入口**: 从 `SettingSection` 调用，用于需要开关控制的设置项
...
- **位置**: `SettingUI.kt`
- **功能**: 可点击跳转的设置项
- **入口**: 从 `SettingSection`、`SupportAndHelpSection` 调用，用于需要跳转的设置项
...
- **位置**: `SettingUI.kt`
- **功能**: 只显示信息的设置项（不可点击）
- **入口**: 从 `SettingSection` 调用，用于只读信息显示
- **组成**: 标题 + 值 + 红点标识（可选）

### 75. SettingDivider（设置分隔线）
- **位置**: `SettingUI.kt`
- **功能**: 设置项之间的分隔线
- **入口**: 从 `SettingSection` 调用，用于分隔不同的设置项
- **样式**: 渐变水平线

### 76. LogoutButton（退出登录按钮）
- **位置**: `SettingUI.kt`
- **功能**: 退出登录的按钮
- **入口**: 从 `SettingContent` 调用，位于设置页面底部
- **样式**: 居中显示文字，带边框容器

### 77. SubscriptionManagementContainer（订阅管理容器）
- **位置**: `SubscriptionUI.kt`
- **功能**: 订阅管理功能的容器组件
- **入口**: 从 `SettingContent` 的 SupportAndHelpSection 调用
- **样式**: 带边框和背景的圆角容器
...
- **位置**: `ChatItem.kt`
- **功能**: 支持样式化的消息文本显示
- **入口**: 从 `ChatItemAI`、`ChatItemUser` 调用，用于显示格式化后的消息文本
- **特征**: 动作文本斜体显示、特殊格式解析
...
- **位置**: `SmartTagsLayout.kt`
- **功能**: 标准样式的标签项
- **入口**: 从 `SmartTagsLayout` 调用，当 `isCardTag` 为 false 时使用
- **样式**: 深色背景，白色边框渐变，较大字体

### 81. LiteTagItem（精简标签项）
- **位置**: `SmartTagsLayout.kt`
- **功能**: 精简样式的标签项（用于卡片）
- **入口**: 从 `SmartTagsLayout` 调用，当 `isCardTag` 为 true 时使用
- **样式**: 深色背景，紫色边框渐变，较小字体

### 82. SettingDialogs（设置对话框）
- **位置**: `SettingContent.kt`
- **功能**: 设置页面相关的对话框集合
- **入口**: 从 `SettingContent` 调用，根据 `dialogState` 显示不同的对话框
- **类型**: 删除账号确认对话框等

### 83. SupportAndHelpSection（支持与帮助区域）
- **位置**: `SettingContent.kt`
- **功能**: 设置页面中的支持与帮助功能区域
- **入口**: 从 `SettingContent` 调用，位于设置页面顶部
