 ...
 *
 * 将字符串作为事件名称，便捷地上报 Firebase Analytics 事件。
 * 内部调用 [FirebaseManager] 进行安全的参数处理和事件发送。
 *
 * 使用示例:
 * ```kotlin
 * // 无参数事件
 ...
 * @param params 事件参数键值对，值支持 String、Int、Long、Double 等基础类型，
 *               null 值会被 [FirebaseManager.safeEventParams] 安全过滤
 */
fun String.logEvent(vararg params: Pair<String, Any?>) {
    FirebaseManager.logEvent(
        this, FirebaseManager.safeEventParams(*params)
    )
}