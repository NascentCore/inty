...
// 其他异常需要记录
    ...
    protected fun launchPersistent(
        ...
        return CoroutineScope(SupervisorJob() + Dispatchers.IO + context + exceptionHandler).launch {
            executeBlock(block, "持久化协程")
        }
    }
...
//区域辅助方法
    ...
    private fun launchCoroutine(
        ...
        return scope.launch(context + exceptionHandler) {
            executeBlock(block, "协程", onError)
        }.apply {
            jobList.add(this)
            invokeOnCompletion {
                jobList.remove(this)
