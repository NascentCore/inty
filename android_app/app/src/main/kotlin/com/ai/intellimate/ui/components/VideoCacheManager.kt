...
/** 视频缓存管理器 提供视频文件的本地缓存功能，优化加载性能 */
class VideoCacheManager private constructor(private val context: Context) {

    companion object {
        @Volatile private var INSTANCE: VideoCacheManager? = null
        ...
        withContext(Dispatchers.IO) {
            ...
            if (memoryCachedPath != null) {
                ...
                if (cachedFile.exists()) {
                    LogUtils.d(
                        "VideoCacheManager - [getVideoPath] ✓ 使用内存缓存: $url -> $memoryCachedPath, size=${cachedFile.length()}"
                    )
            ...
            if (storedPath != null) {
                ...
                if (storedFile.exists()) {
                    ...
                    LogUtils.d(
                        "VideoCacheManager - [getVideoPath] ✓ 使用持久化缓存: $url -> $storedPath, size=${storedFile.length()}"
                    )
            ...
            LogUtils.d(
                "VideoCacheManager - [getVideoPath] 检查文件缓存: ${cachedFile.absolutePath}, exists=${cachedFile.exists()}"
            )
            if (cachedFile.exists()) {
                ...
                LogUtils.d(
                    "VideoCacheManager - [getVideoPath] ✓ 使用文件缓存: $url -> ${cachedFile.absolutePath}, size=${cachedFile.length()}"
                )
                return@withContext cachedFile.absolutePath
            }
        ...
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                if (memoryCachedPath != null || cachedFile.exists()) {
                    ...
                    if (cachedFile.exists()) {
                        LogUtils.d(
                            "VideoCacheManager - [preloadVideo] 缓存文件: ${cachedFile.absolutePath}, size=${cachedFile.length()}"
                        )
                    }
                    return@withContext
                }
                ...
                if (response.isSuccessful) {
                    ...
                    if (body != null) {
                        ...
                        val cachedFile = getCachedFile(cacheKey)
                        LogUtils.d(
                            "VideoCacheManager - [preloadVideo] 保存到: ${cachedFile.absolutePath}"
                        )
                            ...
                            FileOutputStream(cachedFile).use { output ->
                                ...
                                while (input.read(buffer).also { bytesRead = it } != -1) {
                                    ...
                                    if (
                                        ...
                                        LogUtils.d(
                                            "VideoCacheManager - [preloadVideo] 下载进度: $downloadedBytes/$contentLength bytes ($progress%)"
                                        )
                                    }
                                }
                            }
                        ...
                        LogUtils.d(
                            "VideoCacheManager - [preloadVideo] 文件大小: ${cachedFile.length()} bytes"
                        )
                        LogUtils.d(
                            "VideoCacheManager - [preloadVideo] 下载时间: ${downloadTime}ms, 总时间: ${totalTime}ms"
                        )
                ...
                val totalTime = System.currentTimeMillis() - startTime
                LogUtils.e(
                    "VideoCacheManager - [preloadVideo] ✗ 预加载异常: $url, error=${e.message}, time=${totalTime}ms"
                )
    ...
    fun isCached(url: String): Boolean {
        ...
        LogUtils.d(
            "VideoCacheManager - [isCached] URL: $url, memoryCached=$memoryCached, fileCached=$fileCached, result=$result"
        )
        return result
    }
