...
/**
 * Credential Manager 助手类使用最新的 Credential Manager API 进行 Google 登录 参考：
 * https://developer.android.google.cn/identity/sign-in/credential-manager-siwg?hl=zh-cn
 */
object CredentialManagerHelper {

    /**
     * 使用 Credential Manager 和 GetSignInWithGoogleOption 进行 Google 登录（支持现有账户和新用户注册）
...
// https://developer.android.com/identity/sign-in/credential-manager
// 凭据管理器还提供了其他几种登录和注册方式：
// 通行密钥（Passkeys）：用户创建和使用密码的替代方案，提供更安全的登录体验。
// 联合登录（联合身份登录）：通过Google、Facebook等进行登录，简化了注册和登录流程。
// 密码（Password）：支持传统的用户名和密码登录方式。
                val credentialManager = CredentialManager.create(context)
//创建使用Google登录选项
// 注意：GetSignInWithGoogleOption 必须是 GetCredentialRequest 中的唯一选项
// GetGoogleIdOption 对比获取SignInWithGoogleOption：
// https://stackoverflow.com/a/78840062
// GetGoogleIdOption 用于创建“使用Google账号登录”流程
// GetSignInWithGoogleOption 用于触发“使用Google账号登录”按钮流程
