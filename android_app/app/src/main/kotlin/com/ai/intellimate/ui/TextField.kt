    ...
    Row(modifier = modifier.wrapContentHeight(), verticalAlignment = Alignment.CenterVertically) {
        ...
        Box(
            ...
            var textFieldValue by remember {
                mutableStateOf(TextFieldValue(value, selection = TextRange(selection.coerceAtMost(value.length))))
            }

            // 使用LaunchedEffect来监听外部value和selection的变化
            LaunchedEffect(value, selection) {
                textFieldValue = textFieldValue.copy(text = value, selection = TextRange(selection.coerceAtMost(value.length)))
            }
