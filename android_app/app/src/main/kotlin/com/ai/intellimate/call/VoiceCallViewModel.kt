...
class VoiceCallViewModel(
    private val repository: AICallRepository
) : ViewModel() {
    // UI状态
    ...
    private val _audioSendQueue = Channel<ByteArray>(
    ...
    init {
        // 启动队列消费协程
        sendQueueJob = viewModelScope.launch(Dispatchers.IO) {
    ...
    fun startCalling(agentId: String) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                _uiState.update {
                    it.copy(agent = AgentStore.getAgent(agentId))
                }
                    ...
                    .collect { packet ->
                        when (packet.type) {
                            ...
                            CallType.ERROR -> {
                                ...
                                _uiState.update {
                                    it.copy(connectionState = ConnectionState.ERROR)
                                }
                            }
                            CallType.STATUS -> {
                                try {
                                    ...
                                    _uiState.update {
                                        it.copy(callState = packet.status)
                                    }
                                } catch (e: Exception) {
        ...
        viewModelScope.launch {
            try {
                ...
                if (queueUsage >= UiConfigs.VoiceCall.QUEUE_WARNING_THRESHOLD && 
                    ...
                    LogUtils.w("发送队列使用率较高: ${(queueUsage * 100).toInt()}% (约${sendQueueSize}/${UiConfigs.VoiceCall.MAX_SEND_QUEUE_SIZE})")
