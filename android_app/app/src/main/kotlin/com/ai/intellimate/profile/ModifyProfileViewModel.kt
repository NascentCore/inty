...
class ModifyProfileViewModel : BaseVM() {
    ...
    fun setUserAppearance(uri: Uri) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            try {
                // 将 URI 转换为临时文件
                tempFile = createTempFileFromUri(uri) ?: run {
                    NetworkErrorHandler.showNetworkAwareError("Failed to read image file")
                    return@launch
                }

                // 转换为 JPG 并压缩到 2MB 以内
                compressedFile = convertToJpgAndCompress(tempFile, maxSizeKB = 2048) ?: run {
                    NetworkErrorHandler.showNetworkAwareError("Failed to compress image")
                    return@launch
                }

                // 更新用户外观
                _userProfile.value = repository.updateUserAppearance(_userProfile.value, compressedFile)
            ...
            _isAppearanceUploading.value = false
        }
    }
     ...
     */
    private suspend fun convertToJpgAndCompress(
        imageFile: File,
        maxSizeKB: Int = 2048
        ...
        try {
            ...
            if (originalSizeKB <= maxSizeKB) {
                // 尝试直接读取并转换为 JPG
                bitmap = BitmapFactory.decodeFile(imageFile.absolutePath) ?: return@withContext null
                val jpgFile = File(Utils.getApp().cacheDir, "appearance_${System.currentTimeMillis()}.jpg")
            ...
            bitmap = BitmapFactory.decodeFile(imageFile.absolutePath, decodeOptions) ?: return@withContext null
            ...
            if (finalSizeKB > maxSizeKB) {
                LogUtils.w("Compressed image still exceeds ${maxSizeKB}KB limit: ${finalSizeKB}KB")
            }
