 ...
 * 用户服务
 * 封装所有用户相关的API调用
 * 替换原有的 IUserApi 用户相关方法
 */
object UserService {

    /**
     * 获取用户信息
     * 替换: IUserApi.getUserProfile()
     */
    ...
    suspend fun updateUserProfile(userProfile: UserProfile): ApiResult<UserProfile> {
        return IntyNetworkManager.executeRequest("Update User Profile") {
            ...
            val response = IntyNetworkManager.getClient()
                .api().v1().users().profile().update(updateParams)
    ...
    /**
     * 上传头像
     * 替换: IUserApi.uploadAvatar()
     * 注意: 当前 IntySDK 没有直接的头像上传 API，需要通过通用图片上传实现
     */
    ...
    /**
     * 删除用户账户
     * 替换: IUserApi.deleteUser()
     * 注意: 当前 IntySDK 没有直接的 delete user API
     */
