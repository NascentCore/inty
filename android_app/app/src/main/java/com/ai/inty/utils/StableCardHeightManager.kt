...
/** 稳定的卡片高度管理器 在卡片渲染后记录实际尺寸，确保UI复用时不会跳动 */
object StableCardHeightManager {
    ...
    /** 获取卡片的稳定高度（dp） 基于图片真实比例计算，确保瀑布流效果 */
    ...
    /**
     * 获取图片的显示高度（像素） 直接返回像素值，避免dp转换的精度问题
     *
        ...
        try {
            withContext(Dispatchers.IO) {
                ...
                if (options.outWidth > 0 && options.outHeight > 0) {
                    ...
                    EasyLog.log(
                        "StableCardHeightManager - 预加载图片尺寸: $imageUrl -> ${size.x}x${size.y}"
                    )
                }
            }
        } catch (e: Exception) {
    ...
    /** 根据图片尺寸计算显示高度（像素） 直接返回像素值，避免dp转换的精度问题 */
