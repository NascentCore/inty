...
class ChatViewModel : BaseActivityViewModel() {
  ...
  fun queryMsgs() {
    ...
    if (lastQueryAgentId == currentAgentId && currentTime - lastQueryTime < QUERY_DEBOUNCE_TIME) {
      ...
      try {
        ...
        _msgs.update { currentMsgs ->
          try {
            ...
            EasyLog.log("Error updating messages list: ${e.message}", priority = EasyLog.ERROR)
        ...
        currentAgent?.let { agent ->
          ...
          when (result) {
            is HttpResult.Success -> {
              runCatching {
                    ...
                    _msgs.update { currentMsgs ->
                      try {
                        val newMsgs = mutableListOf<MsgInfo>()
                        result.data.data?.choices?.forEach { choice -> newMsgs.add(choice.message) }
                        // 创建当前消息的副本以避免并发修改
                        newMsgs.addAll(currentMsgs.toList())
                        newMsgs
                      } catch (e: Exception) {
            ...
            ?: run {
              // 如果没有 agent 信息，恢复状态
              _isWaitingForReply.value = false
              EasyLog.log("No agent info available for sending message", priority = EasyLog.ERROR)
            }
      } catch (e: Exception) {
  ...
  private fun getChatSetting() = launchWithNetCheck {
    ...
    when (result) {
      ...
      is HttpResult.Success -> {
        ...
        _chatSettings.update { currentSettings -> currentSettings + (agentId to result.data) }
    ...
    when (result) {
      ...
      is HttpResult.Success -> {
        ...
        result.data.data?.let { chatSettingData ->
          _chatSettings.update { currentSettings -> currentSettings + (agentId to chatSettingData) }
          ...
          is HttpResult.Success -> {
            ...
            if (userInitiatedConversations.isEmpty()) {
              ...
              if (currentConversationsPage == 0) {
                ...
                // 后续页，追加到现有列表
                _conversations.value = _conversations.value + userInitiatedConversations
                EasyLog.log(
