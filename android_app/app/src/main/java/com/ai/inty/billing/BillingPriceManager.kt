        ...
        BillingClient.BillingResponseCode.OK -> {
          skuDetailsList?.let { detailsList ->
            if (detailsList.isNotEmpty()) {
              ...
              eventScope.launch {
                eventFlow.emit(
                    BillingEvent.SkuDetailsQueryFailed(billingResult.responseCode, "查询成功但返回空商品列表")
                )
              }
            }
          }
  ...
  private fun updateLocalPlans(
    ...
    skuDetailsList.forEach { skuDetails ->
      ...
      if (index >= 0) {
        ...
        val correctedPrice = BillingUtils.correctCurrencySymbol(formattedPrice, currencyCode)
