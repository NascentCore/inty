                ...
                modifier =
                    Modifier.background(Color.Black.copy(alpha = 0.5f), msgShape)
                        ...
                        .pointerInput(item.content) {
                          detectTapGestures(
                              onLongPress = { debugOnlyCopyToClipboard(context, item.content) }
                          )
                        }
            ) {
...
private fun ChatItemUser(item: MsgInfo) {
  runCatching {
        Row {
          ...
          Box(
              modifier =
                  Modifier.background(Color.White.copy(alpha = 0.6f), RoundedCornerShape(12.dp))
                      .padding(12.dp, 13.dp)
                      .widthIn(1.dp, 300.dp)
                      .pointerInput(item.content) {
                        detectTapGestures(
  ...
  Box(modifier = modifier.fillMaxWidth()) {
    ...
    Text(
        ...
        onTextLayout = { textLayoutResult ->
          ...
          pd = if (textLayoutResult.lineCount >= 3 && textLayoutResult.hasVisualOverflow) 15 else 0
        },
    )
    if (expandable) {
      Icon(
