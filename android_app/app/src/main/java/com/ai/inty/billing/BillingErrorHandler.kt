...
object BillingErrorHandler {
    ...
    private fun provideSolutionSuggestions(billingResult: BillingResult, context: Context?) {
        val suggestions = when (billingResult.responseCode) {
            BillingClient.BillingResponseCode.BILLING_UNAVAILABLE -> {
                listOf(
        ...
        suggestions.forEach { suggestion ->
            EasyLog.log("BillingErrorHandler - $suggestion")
        }

        // 如果是BILLING_UNAVAILABLE，提供额外的诊断信息
        if (billingResult.responseCode == BillingClient.BillingResponseCode.BILLING_UNAVAILABLE && context != null) {
