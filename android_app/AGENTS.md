...
- 手机相册裁剪完成后，必须立即在页面主预览中展示裁剪结果，不能等待上传接口返回。
- 在裁剪结果上传到后端的过程中，需在预览区域叠加 `ThreeDotLoadingAnimation`（与用户资料头像上传一致的旋转等待动画），直到成功或失败。
- 上传失败时恢复为上一次成功同步到后端的头像，同时通过 Toast 告知用户。
