...
def analyze_prompt_content(prompt: str) -> Dict[str, str]:
    ...
    if not result['personality'] and not result['scenario']:
        ...
        if len(clean_prompt) < 300 and ('你是' in clean_prompt or '扮演' in clean_prompt):
...
async def migrate_single_agent(session: AsyncSession, agent: models.Agent, dry_run: bool = True) -> bool:
    """
