...
- **`document_kind` ↔ 逻辑路径**：`/app/core/agentic_kernel/companion/memory_store_document_mapping.py`（例：`IDENTITY.md` → **`identity`**，`context_json` ↔ `context.json`，`transcript` ↔ `transcript.jsonl`）。

## 作用域三元组怎么对齐

- **`companion_id`**：HTTP/API/WebSocket 里的 **`agent_id` 原样**（见 `/app/core/agentic_kernel/companion/AGENTS.md`）。
...
- 持久化约定：**`/app/core/agentic_kernel/companion/AGENTS.md`**「持久化与数据表」「进程内 registry」小节。
