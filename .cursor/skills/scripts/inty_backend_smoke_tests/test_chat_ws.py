...
创建新 agent 再测（无需事先填写 agent_id）:
  python3 .cursor/skills/scripts/inty_backend_smoke_tests/test_chat_ws.py --api-base http://127.0.0.1:8001 --create-agent
"""
...
def main() -> None:
    ...
    p.add_argument(
        "--config",
        "-c",
        help="YAML config (see .cursor/skills/scripts/inty_backend_smoke_tests/config.example.yaml)",
    )
    p.add_argument(
        "--api-base",
